# Mystral — архив прогресса разработки

## 2026-06-18 — TZ-001: Инициализация проекта

- **Сделано:**
  - Создана полная структура проекта (43 файла)
  - `backend/` — FastAPI + SQLModel + Alembic + asyncpg + redis
    - `app/models/user.py` — User, AuthProvider, UserProfile, UserPartner (UUID PK, SQLModel)
    - `app/core/` — config (pydantic-settings), database (async engine + session), redis (asyncio client)
    - `app/api/v1/health.py` — GET /health → {"status": "ok", "service": "mystral"}
    - `app/api/v1/auth.py` — заглушки /auth/telegram, /refresh, /logout
    - `app/migrations/` — Alembic env.py с async engine, script.py.mako
    - `backend/Dockerfile` — запуск: alembic upgrade head → uvicorn
  - `frontend/` — React 18 + Vite + TypeScript + Tailwind + i18next
    - `src/i18n/` — ru.json + en.json, initReactI18next
    - `src/utils/telegram.ts` — isTMA(): boolean
    - `package.json` — все зависимости прописаны
  - `bot/` — aiogram 3.x skeleton, /start команда с WebApp кнопкой
  - `nginx/nginx.conf` — proxy → backend:8000, frontend:5173, WebSocket upgrade
  - `docker-compose.yml` — postgres:16 + redis:7 + backend + frontend + bot + nginx
    - healthcheck на postgres и redis, depends_on с condition: service_healthy
  - `.env.example` + `.env` (скопирован из example)
  - `.gitignore` — Python, Node, env, IDE

- **Найдено проблем:**
  - Нет. Структура чистая, нет circular imports.

- **Следующий шаг:**
  - TZ-002 (дизайн-система): см. ниже

## 2026-06-18 — TZ-002: Дизайн-система Mystral (фикс Dockerfile + дизайн-токены)

- **Сделано:**
  - `backend/Dockerfile` — Python 3.11-slim, добавлены build-essential/gcc/libssl-dev для pyswisseph
  - `tailwind.config.ts` — кастомная палитра: bg/violet/gold/text/border + шрифты display/sans
  - `src/styles/globals.css` — CSS-переменные, Google Fonts (Cormorant Garamond), body стили
  - `src/components/ui/Button.tsx` — variant: primary/gold/ghost, size: sm/md
  - `src/components/ui/Card.tsx` — glassmorphism карточка с border-subtle
  - `src/components/ui/Badge.tsx` — FREE (violet) / PRO (gold)
  - `src/components/ui/BottomNav.tsx` — фикс. навигация, 4 пункта, активный элемент с точкой (lucide-react)
  - `src/components/ui/index.ts` — реэкспорт всех UI компонентов
  - `src/App.tsx` — тёмный фон, "✦ Mystral" Cormorant Garamond, max-w-[390px], BottomNav
  - `package.json` — добавлен lucide-react

- **Найдено проблем:**
  - TS ошибка: кастомный тип иконки не совместим с LucideIcon (size: string|number vs number) — исправлен импортом LucideIcon напрямую

- **Проверено:**
  - `tsc --noEmit` — 0 ошибок
  - `vite build` — ✓ 1538 modules, 3.11s

- **Следующий шаг:**
  - TZ-003 (Three.js + Home): см. ниже

## 2026-06-18 — TZ-003: Главный экран — Three.js орба + layout

- **Сделано:**
  - `package.json` — добавлены three@^0.167.0 + @types/three@^0.167.0
  - `src/components/three/ZodiacOrb.tsx` — Three.js орба:
    - 120 звёзд (PointsMaterial opacity 0.6)
    - SphereGeometry orb (violet-600, emissive)
    - 2 кольца TorusGeometry с точками-спутниками
    - AmbientLight 0.3 + PointLight violet
    - requestAnimationFrame с camera.position.y oscillation
    - renderer.dispose() cleanup
    - HTML overlay: символ знака по центру canvas (font-display 56px)
  - `src/components/ui/BottomNav.tsx` — добавлен optional `active` prop (controlled/uncontrolled)
  - `src/pages/Home.tsx` — полный главный экран:
    - Header 46px с backdrop-blur, пульсирующая точка
    - Приветствие с font-display
    - ZodiacOrb (Скорпион ♏) по центру
    - Карточка гороскопа с AI-бейджем
    - Сетка 2×3 инструментов (Таро/Луна/Натальная/Совместимость/Нумерология/Руны)
  - `src/App.tsx` — рендерит <Home />

- **Найдено проблем:**
  - Предупреждение Vite: чанк 667KB (Three.js). Не ошибка, ожидаемо. При необходимости — dynamic import().

- **Проверено:**
  - `tsc --noEmit` — 0 ошибок
  - `vite build` — ✓ 1541 modules, 5.22s

- **Следующий шаг:**
  - TZ-004 (Таро): см. ниже

## 2026-06-18 — TZ-004: Экран Таро — 3D flip + расклад

- **Сделано:**
  - `src/data/tarot.ts` — 22 карты Старших Арканов + тип TarotCardData + drawCards()
  - `src/components/tarot/TarotCard.tsx` — CSS 3D flip (perspective/preserve-3d/backfaceVisibility):
    - Рубашка: gradient #1B0C4A→#080316, золотой ✦ + MYSTRAL 8px
    - Лицевая: gradient #1E0E50→#0D0520, номер/символ/название, инсет-рамка
    - Transition 0.7s ease с поддержкой delay prop
  - `src/pages/Tarot.tsx` — полный экран расклада:
    - 3 карты с rotations [-8,0,8]°, клик по карте открывает её
    - Кнопка "Получить толкование" появляется после открытия всех 3
    - Имитация стриминга (посимвольно, 30ms задержка)
    - Кнопка "Новый расклад" — перетасовка и сброс
  - `src/components/ui/BottomNav.tsx` — добавлен `onNavigate` prop
  - `src/pages/Home.tsx` — клик по тулам вызывает onNavigate(tool.id)
  - `src/App.tsx` — useState роутинг: home | tarot | moon | profile

- **Проверено:**
  - `tsc --noEmit` — 0 ошибок
  - `vite build` — ✓ 64 modules, 2.92s

- **Следующий шаг:**
  - TZ-005 (Groq): см. ниже

## 2026-06-18 — TZ-005: Groq horoscope endpoint

- **Сделано:**
  - `backend/requirements.txt` — добавлен groq==0.9.0
  - `backend/app/api/v1/horoscope.py` — SSE стриминг endpoint:
    - POST /v1/horoscope/stream
    - Модель: llama-3.3-70b-versatile
    - Поддержка lang: ru/en с разным tone prompt
    - 22 знака зодиака (en→ru маппинг)
    - StreamingResponse, media_type text/event-stream
    - Формат чанков: `data: {"text": "..."}` + `data: [DONE]`
  - `backend/app/api/router.py` — подключён horoscope_router с prefix="/v1"
  - `.env` — плейсхолдер GROQ_API_KEY= уже был, Саша заполнит вручную

- **Найдено нюансов:**
  - Health endpoint остался на `/health` (без `/v1`). Через nginx: `GET /api/health`.
    Задача упоминала `/api/v1/health` — это опечатка, реальный путь `/api/health`.
  - Groq streaming использует sync iterator внутри async generator — корректно для
    данного объёма запросов, при необходимости можно обернуть в run_in_executor.

- **Следующий шаг:**
  - TZ-006 (Groq фронтенд): см. ниже

## 2026-06-18 — TZ-006: Подключение реального Groq к фронтенду

- **Сделано:**
  - `frontend/src/utils/api.ts` — SSE хелпер streamRequest():
    - fetch + ReadableStream reader, TextDecoder
    - Парсинг `data: {...}` / `data: [DONE]`
    - VITE_API_URL из import.meta.env (пустая строка = same origin)
  - `frontend/src/vite-env.d.ts` — добавлен (не было), `/// <reference types="vite/client" />`
  - `backend/app/api/v1/tarot.py` — POST /v1/tarot/interpret, SSE стриминг, 80-100 слов
  - `backend/app/api/router.py` — подключён tarot_router с prefix="/v1"
  - `frontend/src/pages/Tarot.tsx` — handleInterpret теперь вызывает реальный API
  - `frontend/src/pages/Home.tsx` — useEffect загружает гороскоп при монтировании,
    статус "Звёзды говорят..." → стриминг → финальный текст; дата из new Date()
  - `frontend/.env` — создан, VITE_API_URL= (пустая строка, same origin)

- **Найдено проблем:**
  - TS2339: `import.meta.env` не типизирован — исправлен добавлением `vite-env.d.ts`

- **Проверено:**
  - `tsc --noEmit` — 0 ошибок
  - `vite build` — ✓ 65 modules, 2.72s

- **Следующий шаг:**
  - TZ-007 (Auth): см. ниже

## 2026-06-18 — TZ-007: Авторизация — Telegram TMA + Email + JWT

- **Сделано:**
  - `backend/app/core/security.py` — hash/verify password (bcrypt), create/decode JWT (30 дней),
    validate_telegram_hash (HMAC-SHA256 по Telegram WebApp spec)
  - `backend/app/core/deps.py` — get_current_user (HTTPBearer → JWT → User)
  - `backend/app/core/database.py` — переключён на sqlmodel.ext.asyncio.session.AsyncSession
    (поддержка session.exec(select(...)))
  - `backend/app/models/user.py` — добавлен password_hash: Optional[str] в AuthProvider
  - `backend/app/api/v1/auth.py` — 4 эндпоинта:
    POST /v1/auth/telegram, POST /v1/auth/register, POST /v1/auth/login, GET /v1/auth/me
  - `backend/app/api/router.py` — auth router перенесён под prefix="/v1"
  - `backend/app/main.py` — lifespan вызывает create_db_and_tables() при старте
  - `frontend/src/context/AuthContext.tsx` — TMA auto-login → localStorage → LoginScreen
  - `frontend/src/pages/LoginScreen.tsx` — переключатель Войти/Регистрация, email+pass
  - `frontend/src/App.tsx` — AuthProvider wrapper, loading splash, !user → LoginScreen
  - `frontend/src/pages/Home.tsx` — user?.name вместо хардкода "Александра"

- **Найдено нюансов:**
  - `Redis.aclose()` не существует в redis-py 5.x — ошибка при shutdown, не при старте.
    Фикс: заменить на `await redis_client.close()` в redis.py (не критично сейчас)

- **Проверено:**
  - `tsc --noEmit` — 0 ошибок
  - `docker-compose restart backend` — Application startup complete ✓
  - Таблицы создаются автоматически через create_db_and_tables() при старте

- **Следующий шаг:**
  - Открыть localhost:5173 → LoginScreen → зарегистрироваться → главный экран
  - TZ-009: Лунный календарь (страница Moon)

## 2026-06-18 — TZ-008: Redis кэш + Натальная карта

- **Сделано:**
  - `frontend/src/pages/Home.tsx` — добавлен `useRef(false)` guard в useEffect (StrictMode fix)
  - `backend/app/api/v1/natal.py` — два эндпоинта:
    - POST /v1/natal/calculate — геокодинг Nominatim → kerykeion AstrologicalSubject(online=False)
      → JSON: sun/moon/rising (sign + degree), mercury/venus/mars (sign)
    - POST /v1/natal/interpret — то же + SSE стриминг Groq (100-120 слов, мягкий тон)
    - SIGNS_RU: 3-буквенные и полные названия (Ari/Aries → Овен и т.д.)
  - `backend/app/api/router.py` — natal_router подключён с prefix="/v1"
  - `frontend/src/pages/NatalChart.tsx` — двухшаговый экран:
    - Шаг 1: форма (имя, дата 3-в-ряд, время 2-в-ряд, город)
    - Шаг 2: "Большая тройка" (gold) + планеты + SSE интерпретация
  - `frontend/src/App.tsx` — тип Page: добавлен 'natal', рендер NatalChart
  - `backend/app/core/redis.py` — исправлен `aclose()` → `close()` (redis-py 5.x)

- **Проверено:**
  - `tsc --noEmit` — 0 ошибок
  - `docker-compose restart backend` — Application startup complete ✓
  - aclose() ошибка при shutdown устранена

- **Следующий шаг:**
  - TZ-009: Профиль пользователя ✓ (выполнен ниже)
  - TZ-010: Лунный календарь (страница Moon)
  - Alembic миграции (вместо create_all)

## 2026-06-18 — TZ-009: Профиль пользователя + сохранение данных рождения

- **Сделано:**
  - `backend/app/api/v1/profile.py` — GET /v1/profile + PUT /v1/profile:
    - GET: возвращает UserProfile + completion_percent (filled/4 * 100)
    - PUT: birth_date (str→date), birth_time (str→time), birth_city, birth_name→birth_name_enc, lang→User.lang
    - `_get_or_create`: создаёт пустой профиль если не существует
  - `backend/app/api/router.py` — profile_router подключён
  - `frontend/src/pages/Profile.tsx` — экран профиля:
    - Аватар с первой буквой имени (violet bg)
    - Знак зодиака из birth_date (getZodiacSign helper)
    - Прогресс-бар (completion_percent из API)
    - Форма данных рождения (день/месяц/год, час/мин, город, имя)
    - Переключатель языка RU/EN → PUT /profile {lang}
    - Кнопка "Выйти" → logout()
    - Toast "Сохранено ✦" (2.5 сек)
  - `frontend/src/pages/NatalChart.tsx` — интеграция с профилем:
    - useEffect: GET /profile при открытии → auto-fill полей
    - Чекбокс "Сохранить в профиль" (default true)
    - После расчёта: PUT /profile если checked
  - `frontend/src/App.tsx` — Profile добавлен в импорт и маршрут

- **Проверено:**
  - `tsc --noEmit` — 0 ошибок
  - `docker-compose restart backend` — Application startup complete ✓

- **Следующий шаг:**
  - TZ-010: Монетизация ✓ (выполнен ниже)
  - TZ-011: Лунный календарь (страница Moon)
  - Alembic миграции (вместо create_all)

## 2026-06-18 — TZ-010: Монетизация — лимиты + Telegram Stars + ЮKassa

- **Сделано:**
  - `backend/app/api/v1/tarot.py` — requires auth + daily limit:
    - `tarot_daily:{user_id}:{today}` TTL 86400 → free:1/день, >1 → 402 FREE_LIMIT_REACHED
  - `backend/app/api/v1/natal.py` — requires auth + total limit:
    - `natal_count:{user_id}` (no TTL) → free:3 total, >3 → 402 FREE_LIMIT_REACHED
  - `backend/app/api/v1/payments.py` — 4 эндпоинта:
    - POST /v1/payments/stars/create — createInvoiceLink через Telegram Bot API (XTR)
    - POST /v1/payments/stars/confirm — обновляет subscription_tier='pro'
    - POST /v1/payments/yukassa/create — создаёт платёж YuKassa, возвращает payment_url
    - POST /v1/payments/yukassa/webhook — обрабатывает payment.succeeded
  - `backend/app/core/config.py` + `.env` — YUKASSA_SHOP_ID, YUKASSA_SECRET_KEY
  - `backend/app/api/v1/auth.py` — tier добавлен в _user_response и /auth/me
  - `backend/app/api/router.py` — payments_router подключён
  - `frontend/src/utils/api.ts` — streamRequest: 5й аргумент token, 402→{code}; apiRequest<T>
  - `frontend/src/context/AuthContext.tsx` — UserData.tier; updateUser()
  - `frontend/src/components/PaywallSheet.tsx` — bottom sheet, Stars/YuKassa flow
  - `frontend/src/pages/Tarot.tsx` — token в streamRequest; FREE_LIMIT_REACHED → paywall
  - `frontend/src/pages/NatalChart.tsx` — то же для интерпретации
  - `frontend/src/pages/Profile.tsx` — free→кнопка Pro; pro→бейдж

- **Проверено:**
  - `tsc --noEmit` — 0 ошибок
  - `docker-compose restart backend` — Application startup complete ✓

- **Следующий шаг:**
  - TZ-011: Production deploy ✓ (выполнен ниже)
  - TZ-012: Лунный календарь (страница Moon)
  - Alembic миграции (вместо create_all)

## 2026-06-18 — TZ-011: Production деплой

- **Сделано:**
  - `docker-compose.prod.yml` — external network shared_infra (sweetsin_default), nginx 8080:80
  - `frontend/Dockerfile.prod` — node:20-alpine build → nginx:alpine serve
  - `frontend/nginx.conf` — SPA routing + /api/ → backend:8000
  - `nginx/nginx.prod.conf` — /api/ → backend, / → frontend
  - `nginx/mystral.conf` — референс конфиг для главного VPS nginx
  - `.env.prod` — шаблон с CHANGE_ME плейсхолдерами (в .gitignore)
  - `.gitignore` — добавлен .env.prod
  - `deploy.sh` — git pull → build → up -d; chmod +x

- **Проверено:**
  - `tsc --noEmit` — 0 ошибок

- **Саша делает на VPS:**
  - Купить домен, A-запись → VPS IP
  - `git clone` + заполнить `.env.prod`
  - `docker network ls` → обновить `shared_infra.name` в docker-compose.prod.yml
  - `sudo certbot --nginx -d mystral.app`
  - `bash deploy.sh`

## 2026-06-19 — TZ-012: Связывание аккаунтов email ↔ Telegram

- **Сделано:**
  - `backend/app/core/security.py` — добавлен `validate_telegram_widget()`:
    - Telegram Login Widget использует SHA256(bot_token) как ключ (в отличие от TMA: HMAC nested)
  - `backend/app/api/v1/auth.py` — полная доработка:
    - `TelegramAuthRequest` — поддержка `init_data` (TMA) и `widget_data` (Login Widget)
    - `_resolve_tg_id()` — хелпер: валидирует любой формат, возвращает tg_id
    - POST /auth/telegram — добавлен `is_new: bool` в ответ
    - GET /auth/me — добавлен `providers: list[str]` (запрос к AuthProvider)
    - POST /auth/link-email — привязывает email к текущему аккаунту (requires auth)
    - POST /auth/link-telegram — привязывает TG к текущему аккаунту через widget (requires auth)
    - POST /auth/merge — без auth: валидирует TG + email/pass, переносит TG-провайдер на email-аккаунт
  - `frontend/src/context/AuthContext.tsx` — добавлены `pendingMerge` и `dismissMerge`:
    - После TMA логина: если `is_new === true` → setPendingMerge(true)
  - `frontend/src/App.tsx` — рендерит `<MergeAccountPrompt>` когда `pendingMerge === true`
  - `frontend/src/pages/LoginScreen.tsx` — кнопка "Войти через Telegram":
    - Открывает popup oauth.telegram.org/auth?bot_id=...
    - Слушает window.message с origin https://oauth.telegram.org
    - POST /auth/telegram с widget_data → login()
  - `frontend/src/components/MergeAccountPrompt.tsx` — новый компонент:
    - Bottom sheet с формой email + пароль
    - POST /auth/merge с init_data из TMA → login() → dismissMerge()
  - `frontend/src/pages/Profile.tsx` — раздел "Связанные аккаунты":
    - useEffect: Promise.all([GET /profile, GET /auth/me]) → заполняет providers[]
    - Email: кнопка "Добавить" → инлайн-форма (email + пароль + подтверждение) → POST /auth/link-email
    - Telegram: кнопка "Привязать" → handleTelegramWebLogin → POST /auth/link-telegram
    - Статусные бейджи "Привязан" (фиолетовый/голубой) когда provider в списке

- **Проверено:**
  - `tsc --noEmit` — 0 ошибок

## 2026-06-19 — TZ-013: Официальный Telegram Login Widget (замена popup)

- **Проблема:** popup oauth.telegram.org/auth давал "Bot domain invalid"
- **Решение:** официальный `<script>` виджет от Telegram

- **Сделано:**
  - `backend/app/core/security.py` — добавлен `validate_telegram_widget_hash(data: dict) -> bool`
    (bool-обёртка над `validate_telegram_widget`, для удобства)
  - `frontend/src/pages/LoginScreen.tsx` — заменён popup на script-виджет:
    - `tgBtnRef = useRef<HTMLDivElement>()` — контейнер для виджета
    - useEffect: регистрирует `window.onTelegramWidgetAuth` → POST /auth/telegram (widget_data)
    - Динамически вставляет `<script data-telegram-login="Mystrallbot" data-size="large" ...>`
    - Cleanup: `delete window.onTelegramWidgetAuth`
    - Убрана кнопка-заглушка, вместо неё `<div ref={tgBtnRef} />`

- **Проверено:**
  - `tsc --noEmit` — 0 ошибок
  - `git push` — запушено в main

- **На VPS:**
  - `git pull && docker compose -f docker-compose.prod.yml up --build -d frontend && docker compose -f docker-compose.prod.yml restart nginx`

- **Следующий шаг:**
  - TZ-014: Онбординг + динамический знак + EN ✓ (выполнен ниже)

## 2026-06-19 — TZ-014: Онбординг + динамический знак + EN локализация

- **Сделано:**
  - **БЛОК А — Онбординг:**
    - `backend/app/api/v1/auth.py` — `_user_response` теперь async, включает `has_birth_date: bool`
      (запрос UserProfile из БД). GET /auth/me тоже возвращает поле.
    - `frontend/src/context/AuthContext.tsx` — `has_birth_date` в UserData,
      `i18n.changeLanguage()` при login/updateUser
    - `frontend/src/components/OnboardingModal.tsx` — bottom sheet при первом входе:
      День/Месяц/Год → PUT /profile → updateUser({has_birth_date:true})
    - `frontend/src/App.tsx` — рендерит OnboardingModal когда user && !has_birth_date
  - **БЛОК Б — Динамический знак:**
    - `frontend/src/utils/zodiac.ts` — getZodiacSign(date) → { sign, symbol, en }
      12 знаков с русскими/английскими названиями + Unicode символами
    - `frontend/src/pages/Home.tsx` — убран хардкод "Скорпион ♏":
      GET /profile при монтировании → getZodiacSign → передаётся в ZodiacOrb и streamRequest.
      Приветствие на основе времени суток (greeting_morning/afternoon/evening/night)
    - `frontend/src/pages/Profile.tsx` — zodiac из zodiac.ts (убран дублирующий getZodiacSign)
  - **БЛОК В — EN локализация:**
    - `frontend/src/i18n/locales/ru.json` — ~100 ключей по 9 неймспейсам
    - `frontend/src/i18n/locales/en.json` — полный английский перевод
    - 8 компонентов с `useTranslation() + t('key')`:
      Home, Tarot, NatalChart, Profile, LoginScreen,
      BottomNav, PaywallSheet, MergeAccountPrompt, OnboardingModal
    - Profile: переключатель языка вызывает `updateUser({lang}) + i18n.changeLanguage()`

- **Проверено:**
  - `tsc --noEmit` — 0 ошибок
  - `git push` — запушено (15 файлов, +691 –298)

- **Следующий шаг:**
  - TZ-015: Недостающие экраны ✓ (выполнен ниже)

## 2026-06-19 — TZ-015: Недостающие экраны — Луна, Совместимость, Нумерология, Руны

- **Сделано:**
  - **Backend — 4 новых API модуля:**
    - `lunar.py` — GET /lunar/today (лунный день, фаза, знак луны, рекомендации, энергия),
      GET /lunar/month (массив дней с фазами). Чистая математика от новолуния 2000-01-06.
    - `compatibility.py` — POST /compatibility/calculate (совместимость по стихиям,
      процент + описание), POST /compatibility/interpret (Pro, SSE Groq)
    - `numerology.py` — POST /numerology/calculate (Life Path, Expression, Soul Urge,
      Personality). Таблицы Пифагора для латиницы и кириллицы. Free: только Life Path.
      POST /numerology/interpret (Pro, SSE Groq)
    - `runes.py` — 24 руны Elder Futhark с RU/EN значениями. POST /runes/draw
      (1 или 3, 30% вероятность переворота, free: 1/день через Redis).
      POST /runes/interpret (SSE Groq)
    - `router.py` — подключены все 4 новых роутера
  - **Frontend — 4 новых страницы:**
    - `LunarCalendar.tsx` — Орба луны (CSS), инфо дня, 3 рекомендации,
      календарь месяца (grid 7 колонок, сегодня подсвечен)
    - `Compatibility.tsx` — 2 шага form→result. Автозаполнение из профиля.
      Процент + прогресс-бар + описание. Pro: SSE интерпретация. Free: paywall.
    - `Numerology.tsx` — Форма с автозаполнением. Life Path крупно + gold.
      Free: остальные числа заблюрены с замком. Pro: все числа + интерпретация.
    - `Runes.tsx` — 3 режима idle→drawing→result. Выбор 1/3 руны (3 = Pro).
      Анимация появления. Карточки с символом, позицией, значением.
    - `App.tsx` — роуты: moon/lunar → LunarCalendar, compat → Compatibility,
      numerology/numero → Numerology, runes → Runes
    - i18n: ~50 новых ключей в ru.json + en.json

- **Проверено:**
  - `tsc --noEmit` — 0 ошибок
  - `git push` — запушено (12 файлов, +1511)

- **Следующий шаг:**
  - TZ-016: Groq промпты ✓ (выполнен ниже)

## 2026-06-20 — TZ-016: Улучшение Groq промптов — качество AI текстов

- **Проблема:** AI генерировал общие поэтические тексты без конкретики

- **Сделано:**
  - `backend/app/core/prompts.py` — новый модуль с system_prompt(lang):
    - RU: "опытный астролог, пишешь точно, без 'нежных ветерков', конкретно о знаке/планете"
    - EN: "experienced astrologer, no vague phrases, specific about sign/planet/aspect"
  - Все 6 Groq-вызовов обновлены:
    - horoscope.py: сфера жизни + конкретный совет + время дня
    - tarot.py: значение каждой карты + связь между ними + практический вывод
    - natal.py: черта характера (Солнце) + эмоции (Луна) + восприятие (Асцендент) + вызов
    - compatibility.py: сила пары + источник конфликтов + практический совет
    - numerology.py: жизненная задача + сильные стороны + вызовы + сфера реализации
    - runes.py: значение каждой руны + общий посыл + рекомендация
  - Каждый вызов теперь: `messages=[{role: "system", ...}, {role: "user", ...}]`
  - Все промпты двуязычные (RU/EN ветвление)

- **Проверено:**
  - `git push` — запушено (7 файлов, +187 –62)
  - На VPS: `docker compose restart backend` (пересборка не нужна)

- **Следующий шаг:**
  - TZ-017: Адаптивный дизайн ✓ (выполнен ниже)

## 2026-06-20 — TZ-017: Адаптивный дизайн под ПК

- **Сделано:**
  - `frontend/src/components/layout/AppLayout.tsx` — обёртка с 3-колоночным layout:
    - Левая панель (220px, sticky, hidden <768px): логотип gold, вертикальная навигация
      (8 пунктов с emoji, active = violet bg), статус подписки внизу
    - Центр (390px fixed): рендерит children, бордеры по бокам
    - Правая панель (220px, sticky, hidden <768px): лунный день, знак зодиака, tier
  - `frontend/src/components/layout/RightPanel.tsx` — правая панель:
    - GET /lunar/today → лунная карточка (фаза, знак, энергия)
    - GET /profile → getZodiacSign() → карточка знака
    - Pro/Free бейдж
  - `frontend/src/components/ui/BottomNav.tsx` — добавлен `md:hidden`
  - `frontend/src/App.tsx` — оборачивает весь контент в `<AppLayout>`
  - 8 страниц: убран `max-w-[390px] mx-auto` (AppLayout контролирует ширину)
  - i18n: добавлены nav ключи для compat/natal/numerology/runes

- **Проверено:**
  - `tsc --noEmit` — 0 ошибок
  - `git push` — запушено (14 файлов, +224)

- **Следующий шаг:**
  - TZ-018: Валидация + защита TMA ✓ (выполнен ниже)

## 2026-06-20 — TZ-018: Валидация форм + защита TMA аккаунта

- **Сделано:**
  - **БЛОК А — Валидация (frontend/src/utils/validate.ts):**
    - validateEmail, validatePassword, validateName, validateCity
    - validateDay/Month/Year (диапазоны), validateDateExists (Date roundtrip)
  - **БЛОК Б — Применение в 6 формах:**
    - LoginScreen: email + password (+ name при register)
    - OnboardingModal: day/month/year + validateDateExists
    - NatalChart: name + date + city, per-field ошибки
    - Profile: date поля при сохранении
    - MergeAccountPrompt: email + password
    - Compatibility: оба блока дат
    - Паттерн: Record<string, string> errors, красный текст под полем, очистка при onChange
  - **БЛОК В — Backend email-validator:**
    - requirements.txt: email-validator==2.1.0
    - auth.py register(): validate_email(check_deliverability=False)
  - **БЛОК Г — Защита TMA:**
    - AuthContext.logout(): isTMA() → WebApp.close() вместо logout
    - Profile: вместо "Выйти" → "Аккаунт привязан к Telegram ✓"
    - i18n: profile.tg_account ключ (RU/EN)

- **Проверено:**
  - `tsc --noEmit` — 0 ошибок
  - `git push` — запушено (12 файлов, +286 –100)

- **На VPS:**
  - `docker compose -f docker-compose.prod.yml up --build -d frontend backend`
  - `docker compose -f docker-compose.prod.yml restart nginx`

- **Следующий шаг:**
  - TZ-019: Rider-Waite карты ✓ (выполнен ниже)

## 2026-06-20 — TZ-019: Rider-Waite карты Таро — реальные изображения

- **Сделано:**
  - `backend/scripts/download_tarot.py` — скрипт скачивания 22 карт с Wikimedia Commons
    - Rider-Waite 1909 (public domain)
    - Исправлены URL для Magician (#1) и Lovers (#6) через Wikipedia API
  - `frontend/public/tarot/0-21.jpg` — 22 изображений Старших Арканов
  - `frontend/src/components/tarot/TarotCard.tsx` — лицевая сторона:
    - `<img src="/tarot/{id}.jpg" objectFit="cover">` вместо emoji+текст
    - Полупрозрачная подпись с названием карты внизу
    - Fallback: скрывает img при ошибке загрузки (onError)
    - Рубашка (✦ MYSTRAL) без изменений

- **Проверено:**
  - `tsc --noEmit` — 0 ошибок
  - `git push` — запушено (24 файла: 22 jpg + script + component)

- **Следующий шаг:**
  - TZ-021: Уведомления ✓ (выполнен ниже)

## 2026-06-20 — TZ-021: Telegram-уведомления — ежедневный гороскоп в 9:00

- **Сделано:**
  - **Модель + миграция:**
    - UserProfile: `notifications_enabled: bool = False`
    - database.py: `ALTER TABLE IF NOT EXISTS` + `get_session_context()` async CM
  - **Scheduler (APScheduler):**
    - scheduler.py: каждые 5 минут проверяет кому сейчас 9:00 в их таймзоне
    - Генерирует гороскоп через services/horoscope.py (zodiac_from_date + Groq non-streaming)
    - Отправляет через aiogram Bot, Redis dedup (25h TTL)
    - main.py: scheduler.start() в lifespan
  - **Profile API:**
    - ProfileUpdate: + notifications_enabled, timezone (с валидацией zoneinfo)
    - POST /profile/toggle-notifications — для бота (без JWT, по telegram_id)
  - **Bot:**
    - /notifications команда: toggle через HTTP к backend
  - **Frontend:**
    - OnboardingModal: 2 шага — дата рождения → уведомления (timezone selector)
    - Profile: карточка "Уведомления" с toggle + dropdown таймзоны
    - constants/timezones.ts: 21 таймзона (Россия, СНГ, мир)
    - i18n: все ключи (RU/EN)

- **Проверено:**
  - `tsc --noEmit` — 0 ошибок
  - `git push` — запушено (14 файлов, +461)

- **На VPS:**
  - `docker compose -f docker-compose.prod.yml up --build -d`

- **Следующий шаг:**
  - Alembic миграции (вместо create_all)
  - Тестирование уведомлений на VPS

## 2026-07-05 — TZ-052: SVG натальное колесо

- **Сделано:**
  - `backend/app/api/v1/natal.py` — `build_full_chart()`: куспиды домов теперь содержат
    `abs_pos` (абсолютный градус 0-360), вычисленный через уже существующий `_get_abs_pos()`
    (раньше в объекте дома был только `degree` — позиция внутри знака). Планеты уже
    отдавали `abs_pos` и раньше, но во фронтовом типе `PlanetData` это поле не было объявлено.
  - `frontend/src/components/NatalWheel.tsx` — новый компонент, полностью на SVG,
    без canvas и сторонних либ:
    - Слой 1: пояс из 12 знаков зодиака (сектора-дуги, заливка по стихии, символы)
    - Слой 2: линии домов (угловые 1/4/7/10 — золотые, остальные — белые), номера домов
    - Слой 3: линии аспектов внутри R_inner (только орб < 5°, цвет/толщина по типу аспекта)
    - Слой 4: планеты на R_planet с `resolveCollisions()` — раздвигает символы планет
      ближе 7° друг к другу по радиусу; ретроградные помечены "R"; линия-коннектор к домам
    - Слой 5: градусные метки каждые 10° по внешнему краю
    - Слой 6: декоративный центральный круг
    - Hover-тултип по планете (обычный `div` поверх SVG, не foreignObject):
      "{планета} в {знак} {градус}° · {дом} дом"
    - Анимации: планеты — `mystral-fadeup .6s` с лёгким сдвигом по времени,
      аспекты — новый keyframe `mystral-fadein` (добавлен в `index.css`) с задержкой 0.4s
  - `frontend/src/pages/NatalChart.tsx` — интеграция:
    - `PlanetData`/`HouseData`/`AspectData` дополнены полями `abs_pos`, `planet1`/`planet2`
    - `wheelPlanets`/`wheelHouses`/`wheelAspects` (useMemo) конвертируют данные `chart`
      в формат `NatalWheel` (planet.name намеренно на англ. ключах — так `NatalWheel`
      сопоставляет планеты в аспектах с `planet1`/`planet2` из бэкенда)
    - `wheelSize`: 480px на десктопе (≥768px), иначе `min(innerWidth-48, 520)`, слушает resize
    - Колесо вставлено над табами AI-интерпретации; на десктопе — grid
      `480px 1fr` (колесо слева, AI-интерпретация справа), на мобиле — одна колонка.
      Старый статичный SVG от kerykeion (`/natal/svg`) не трогали — оставлен как есть.

- **Проверено:**
  - `python -m py_compile backend/app/api/v1/natal.py` — 0 ошибок
  - `tsc --noEmit` — 0 ошибок
  - Визуально в браузере (моковые данные, временный harness, не закоммичен):
    геометрия секторов/домов сверена программно через DOM (все линии аспектов
    ровно на радиусе R_inner*0.95, 4 золотые угловые линии домов + 8 обычных,
    12 подписанных домов, центр-круг радиуса R_inner*0.15), коллизии планет
    (Sun/Mercury 5° друг от друга) раздвигаются корректно, hover-тултип
    показывает верный дом по градусу.

- **Следующий шаг:**
  - Alembic миграции (вместо create_all)
  - Тестирование уведомлений на VPS

## 2026-07-05 — TZ-053: Фикс натального колеса и AI-интерпретации

- **Найдено (внешний аудит + баг-репорт):**
  - AI-интерпретация "смещалась в сторону, отображалась криво" при переключении
    табов — причина: `handleInterpret` в `NatalChart.tsx` не имел защиты от гонки
    запросов (тот же класс бага, что чинили в `CompositeChart.tsx` ранее);
    отдельно — колесо визуально "смещено вниз", т.к. рендерилось ПОСЛЕ 7 других
    карточек и дублирующей статичной SVG-картинки от kerykeion.
  - Ретроградность планет не отображалась на самом кольце (только в таблице).
  - Коллизия глифов при близких градусах превращалась в нечитаемое пятно.

- **Сделано:**
  - `frontend/src/pages/NatalChart.tsx`:
    - Убрана статичная SVG-картинка от kerykeion (`svgContent`, fetch `/natal/svg`,
      `dangerouslySetInnerHTML`) — дублировала функциональность `NatalWheel`.
    - Блок "Натальное колесо + AI-интерпретация" перенесён в начало результата
      (сразу после расчёта), остальные карточки (Стеллиумы/Большая тройка/Планеты/
      Дома/Стихии/Аспекты) идут ниже — устраняет "пустое пространство сверху".
    - AI-интерпретация переведена на явный клик: `selectSection()` только
      переключает таб, `fetchInterpretation()` запускается по кнопке
      "Получить интерпретацию" (gold `#C9A84C`). Состояния: спиннер + "Генерируем
      интерпретацию…" (disabled), готовый текст, либо ошибка с текстом
      "Не удалось сгенерировать интерпретацию, попробуйте снова" и повторной кнопкой.
    - Кеш интерпретаций по разделам на весь просмотр карты: `interpretations:
      Partial<Record<Section,string>>` (React state, без persist в БД).
    - `requestIdRef`-guard в `fetchInterpretation` (тот же паттерн, что и в
      `CompositeChart.tsx`) — чанки устаревшего стрима больше не дописываются
      поверх новой секции.
  - `frontend/src/components/NatalWheel.tsx`:
    - Ретроградный маркер: `R` (красный) → `℞` (`#A89E8B`, приглушённый, как в ТЗ).
    - `resolveCollisions()` переписан: константы `COLLISION_THRESHOLD_DEG = 4`,
      `RADIAL_STEP_PX = 15`; планеты сортируются по градусу, группируются по
      порогу (с учётом перехода через 360°/0° — кластер на стыке склеивается
      в одну группу), каждая планета внутри группы отодвигается на
      `RADIAL_STEP_PX` ближе к центру относительно предыдущей (было: только
      2 уровня радиуса без учёта wraparound).
    - Для каждой раздвинутой (collided) планеты — тонкая линия-указатель
      (0.5px, `#6E6757`) от глифа до её истинного градуса на внешней шкале.

- **Проверено:**
  - `tsc --noEmit` — 0 ошибок (backend не менялся в этом ТЗ)
  - `python -m pytest` — 86/86 (регрессий нет)
  - Визуально (моковые данные, временный harness, не закоммичен): 3-плане́тный
    кластер (Sun/Mercury/Venus, 12-17°) и wraparound-кластер (Neptune 359°/
    Pluto 1°) дают ровно 5 линий-указателей на радиусах 156/141/126px (шаг
    ровно 15px); ретро-маркеры (Mars, Uranus) — `℞` цвета `#A89E8B`.

- **Следующий шаг:**
  - Alembic миграции (вместо create_all)
  - Тестирование уведомлений на VPS

## 2026-07-05 — TZ-053 доп. фикс: layout колеса и AI-интерпретации на десктопе

- **Найдено (скриншот с прода):** side-by-side grid (`480px 1fr`: колесо слева,
  AI-интерпретация справа) выглядел криво — панель интерпретации слишком узкая,
  текст визуально обрезался у правого края колонки.
- **Сделано:** `frontend/src/pages/NatalChart.tsx` — убрал grid-обёртку
  `grid-cols-1 md:grid-cols-[480px_1fr]`. Теперь колесо и карточка
  AI-интерпретации — простые последовательные блоки на всю ширину в общем
  вертикальном потоке (как остальные карточки страницы): колесо сверху,
  ниже — Стеллиумы/Большая тройка/Планеты/Дома/Стихии/Аспекты, и в самом
  низу — AI-интерпретация. Одинаково для desktop и mobile.
- **Проверено:** `tsc --noEmit` — 0 ошибок; backend не менялся.
- **Следующий шаг:**
  - Alembic миграции (вместо create_all)
  - Тестирование уведомлений на VPS

## 2026-07-05 — TZ-055: Критический баг — таймзона не учитывалась при расчёте домов/асцендента

- **Причина (подтверждено чтением исходников kerykeion 4.14.0):** kerykeion
  умеет верно конвертировать местное время рождения в UTC —
  `pytz.timezone(self.tz_str).localize(naive_datetime)` в
  `astrological_subject.py`, с учётом исторических переходов на летнее время.
  Но во ВСЕХ местах приложения, где строится `AstrologicalSubject` из
  реальных данных рождения, было жёстко `tz_str="UTC"` — то есть local time
  проходило как «уже UTC» без какой-либо конвертации. Отсюда неверные
  Асцендент/MC/куспиды домов (а значит и распределение планет по домам)
  почти у всех пользователей; сами планеты (знак/градус) искажались
  минимально.
  - `backend/app/api/v1/natal.py` — `_build_subject()`
  - `backend/app/api/v1/compatibility.py` — 4 места: `compat_moon`,
    `compat_synastry`, `compat_composite`, `composite_interpret`
  - **Не трогал:** `backend/app/api/v1/lunar.py` — там `tz_str="UTC"` с
    `lat=0, lng=0` используется для расчёта сегодняшнего ретрограда Меркурия
    (глобальная проверка, не привязана к чьему-то рождению) — там всё верно.
- **Кэширование карты (уточнено перед фиксом):** натальная карта считается
  ДИНАМИЧЕСКИ на каждый запрос из `birth_date/birth_time/birth_city`,
  ничего не кэшируется в БД/Redis — отдельная миграция/скрипт инвалидации
  не нужны, все карты пересчитаются верно автоматически.
- **Сделано:**
  - `backend/requirements.txt` — добавлен `timezonefinder==8.2.4` (offline
    lookup IANA-таймзоны по координатам, без внешних API и рейт-лимитов).
  - `backend/app/api/v1/natal.py` — новая `resolve_timezone(lat, lon) -> str`
    (глобальный `TimezoneFinder()`, инстанцируется один раз). `_build_subject`
    теперь передаёт `tz_str=resolve_timezone(lat, lon)` вместо хардкода —
    дальше конвертацию в UTC делает сам kerykeion через pytz.
  - `backend/app/api/v1/compatibility.py` — тот же `resolve_timezone`
    прокинут во все 4 места построения `AstrologicalSubject` (по одному
    вызову на каждого из пары).
  - `backend/tests/test_natal_timezone.py` — 2 новых теста с точными
    значениями из ТЗ (Асцендент/MC/все 10 планет для Белореченска,
    Асцендент/MC/Солнце/Луна для Тулы), плюс проверка `subj.tz_str ==
    "Europe/Moscow"`. Пропускаются (`skip`) при отсутствии `pyswisseph`
    (Windows) — реально считают только там, где есть настоящий kerykeion
    (Docker/Linux), как и остальной проект.
- **Проверено:**
  - Оба контрольных кейса — **точное совпадение** со значениями из ТЗ,
    посчитано вручную в Docker (`docker compose run backend python -c
    "..."`) и через сами pytest-тесты:
    - Белореченск 18.11.2003 02:30 → `tz=Europe/Moscow`, Асцендент Дева
      29.2°, MC Близнецы 29.1°, Солнце Скорпион 25.2°, Луна Дева 4.9°,
      Меркурий Стрелец 8.6°, Венера Стрелец 18.7°, Марс Рыбы 14.7°,
      Юпитер Дева 15.6°, Сатурн Рак 12.8° ℞, Уран Водолей 28.9°, Нептун
      Водолей 10.6°, Плутон Стрелец 18.9° — всё сошлось.
    - Тула 28.03.2005 18:30 (день после перехода на летнее время в РФ) →
      `tz=Europe/Moscow`, offset правильно UTC+4 (не хардкод +3): Асцендент
      Дева 24.0°, MC Близнецы 21.9°, Солнце Овен 8.0°, Луна Скорпион 11.1°.
  - Доп. города (offset меняется предсказуемо для каждого): Владивосток
    2015 → Asia/Vladivostok +10; Новосибирск 1998 → Asia/Novosibirsk +6
    (до реформы поясов); Нью-Йорк июль 2020 → America/New_York EDT −4;
    Токио → Asia/Tokyo +9 без DST; Екатеринбург 2003 → Asia/Yekaterinburg
    +6 (историческое значение до реформы 2010/2011).
  - `python -m py_compile` — 0 ошибок (natal.py, compatibility.py,
    test_natal_timezone.py).
  - `pytest` в Docker (реальный kerykeion): **86 passed** (84 старых + 2
    новых), **2 failed** — но это `test_horoscope_scores_valid` и
    `test_logout_blacklists_token`, никак не связанные с натальной картой;
    оба проходят по отдельности, падают только в полном прогоне и только
    на Linux (на Windows тот же прогон — все 86 зелёные). Похоже на утечку
    event loop между тестами в связке pytest-asyncio+fakeredis на Linux —
    предсуществующая проблема тестовой инфраструктуры, не регрессия от
    этого фикса. Стоит завести отдельный тикет.
  - `pytest` локально на Windows: 86 passed, 2 skipped (новые тесты
    корректно пропускаются без pyswisseph).
  - Frontend не менялся, `tsc` не требовался.
- **Следующий шаг:**
  - Отдельно разобраться с падением `test_horoscope_scores_valid` /
    `test_logout_blacklists_token` на Linux в полном прогоне (не блокирует
    этот фикс, но стоит поймать до следующего CI на Linux)
  - Alembic миграции (вместо create_all)
  - Тестирование уведомлений на VPS

## 2026-07-05 — TZ-056: Утечка event loop в тестах на Linux

- **Причина (подтверждено чтением исходников pytest-asyncio 1.4.0):**
  `tests/conftest.py` держал старую `@pytest.fixture(scope="session") def
  event_loop(): ...` — механизм переопределения event loop из старых версий
  pytest-asyncio. В установленной версии (1.4.0) этот механизм полностью
  убран: плагин больше даже не ищет фикстуру с именем `event_loop`, только
  `event_loop_policy`. Реально использовавшееся значение —
  `asyncio_default_test_loop_scope=function` (дефолт библиотеки), то есть
  **каждый тест получал собственный новый event loop**, а старая
  session-фикстура была мёртвым кодом, молча проглоченным
  `filterwarnings = ignore::DeprecationWarning` в `pytest.ini`.
  Модульные singleton-клиенты Redis (`app_redis.redis_client` в
  `app/core/redis.py`, отдельный `redis_client` в `horoscope.py` и т.д.)
  создаются один раз при импорте и живут на весь прогон. Внутреннее
  соединение/очередь fakeredis при первом реальном использовании
  привязывается к тому event loop, что активен в этот момент. В полном
  прогоне более ранние тесты успевали "закрепить" соединение за своим
  (уже закрытым к этому моменту) loop, и следующий тест, попадающий на то
  же соединение под новым loop, падал с `RuntimeError: ... is bound to a
  different event loop`. В изоляции первый же вызов создавал соединение
  под "правильным" (текущим) loop — поэтому тесты проходили по отдельности.
- **Сделано:**
  - `backend/pytest.ini` — добавлены `asyncio_default_fixture_loop_scope
    = session` и `asyncio_default_test_loop_scope = session`: теперь все
    тесты и async-фикстуры в рамках прогона используют один и тот же
    event loop, как и предполагала (но больше не реализовывала) старая
    фикстура.
  - `backend/tests/conftest.py` — убрана мёртвая фикстура `event_loop` и
    ставший неиспользуемым `import asyncio`.
- **Проверено:**
  - Docker/Linux, полный прогон `pytest` (86 старых + 2 из ТЗ-055) —
    **88 passed, 0 failed**, повторено 3 раза подряд (без этого фикса
    было стабильно 86 passed + 2 failed в каждом полном прогоне).
  - Прогон с `-W error::DeprecationWarning` — 88 passed, других
    предупреждений, которые мог маскировать `ignore::DeprecationWarning`,
    не всплыло.
  - Windows локально: 86 passed, 2 skipped — без регрессий.
- **Следующий шаг:**
  - Alembic миграции (вместо create_all)
  - Тестирование уведомлений на VPS

## 2026-07-05 — TZ-054: Пререндер публичных SEO-страниц — узкий фикс после ревизии посылки ТЗ

- **Посылка ТЗ не подтвердилась при проверке прода:** ТЗ исходило из того,
  что все 126 публичных SEO-страниц отдают пустой `<div id="root">` и
  требуют выполнения JS. Проверка `curl https://mystral.space/zodiac/aries`
  (и /tarot/*, /runes/*, /numerology/*) показала, что эти страницы уже
  ПОЛНОСТЬЮ рендерятся сервером: `backend/app/api/v1/seo_pages.py` отдаёт
  готовый Jinja2 HTML (`h1`, абзацы, JSON-LD Article/FAQPage/BreadcrumbList,
  canonical, hreflang) без единой строки JS, а `nginx/nginx.prod.conf` уже
  явно проксирует `/zodiac`, `/tarot`, `/runes`, `/numerology`,
  `/sitemap.xml` на backend в обход SPA-фолбэка. Плюс во фронтенде вообще
  нет `react-router-dom` (роутинг — `useState` в `App.tsx`), так что
  `vite-react-ssg` из Модуля 2 ТЗ было физически не к чему подключать для
  этих страниц.
  - Реальная пустая SPA-оболочка — только у главной `/` (в sitemap с
    приоритетом 1.0, отдаёт `<title>Mystral</title>` и дальше пустой
    `#root`).
  - Отдельно нашёл: `?lang=es/pt/tr/uk` на SEO-страницах отдаёт тот же
    русский контент (`<html lang="ru">`) — hreflang-альтернативы сейчас
    битые. Не чинил — это релевантно TZ-037c (615 страниц, 5 языков), не
    этому тикету.
  - Согласовал с Сашей: узкий фикс — пререндерить только `/`, не трогая
    роутинг/сборку/nginx.
- **Сделано:** `frontend/index.html` —
  - Добавлены `<meta name="description">`, `<link rel="canonical">`,
    JSON-LD `WebSite`.
  - Внутрь `<div id="root">` положен настоящий статический HTML (`h1`,
    два абзаца, ссылки на `/zodiac`, `/tarot`, `/runes`) вместо пустого
    контейнера — виден в сыром HTML-ответе любому краулеру без JS.
  - Никакого cloaking: контент одинаков для всех (ботов и людей).
    `ReactDOM.createRoot(...).render(<App/>)` в `main.tsx` не менялся —
    при монтировании React **полностью заменяет** содержимое `#root`
    (не hydrate, а чистый mount), поэтому статический контент
    автоматически и чисто исчезает для реальных пользователей — доп. код
    на очистку не понадобился.
  - Инлайновые стили под тёмную/золотую тему — чтобы не было некрасивой
    незастилизованной вспышки в момент до монтирования React.
- **Проверено:**
  - `curl http://localhost:5173/` (dev-сервер) — статический контент
    присутствует в сыром HTML.
  - Визуально в браузере: страница монтируется в реальный `LoginScreen`
    без визуальных артефактов/мигания, ошибок в консоли нет.
  - `npm run build` — успешно, `dist/index.html` (3.59 kB) содержит
    статический контент.
  - `tsc --noEmit` — 0 ошибок.
  - `pytest` (backend не менялся, прогнан для регрессии) — 86 passed,
    2 skipped.
- **Не делал (сознательно, вне узкого фикса):**
  - Миграция на `react-router-dom` + `vite-react-ssg`/SSG-пайплайн —
    не нужна, целевые 126 страниц уже server-rendered.
  - Правка nginx — `/` и так корректно фолбэчится на `index.html`.
  - Починка hreflang/`?lang=` — отдельная задача, см. TZ-037c.
- **Следующий шаг:**
  - Alembic миграции (вместо create_all)
  - Тестирование уведомлений на VPS
  - При планировании TZ-037c — учесть, что hreflang-альтернативы сейчас
    битые (см. выше), понадобится реальная генерация контента на 5 языках
    с разными URL, а не просто `?lang=` без изменения ответа

## 2026-07-05 — TZ-057: Баг в генерации share-карточек — Натальная карта и Composite Chart без контента

- **Найдено:**
  - **Натальная карта (Problem 1):** `NatalChart.tsx` вызывал `<ShareCard
    type="natal" title=... onClose=.../>` — вообще без данных карты. И даже
    если бы данные передавались, рендер-блок `type==="natal"` в
    `ShareCard.tsx` показывал только один проп `sign` одним большим
    символом — никакой "большой тройки" там не было предусмотрено в принципе.
  - **Composite Chart (Problem 2):** в `ShareCardProps.type` не было
    варианта `"composite"` вообще — `CompositeChart.tsx` вызывал карточку с
    `type="natal"` и `sign="∞"`, поэтому показывался только символ
    бесконечности без результата (нет числового процента для composite —
    это другая техника, у неё есть "главный аспект" и текстовое AI-описание,
    но не показывались и они).
  - **"Stumager" вместо имени (Problem 3):** `userName = user?.name` брал
    `display_name` аккаунта (в данном случае похожий на логин), а не имя,
    введённое для расчётов. Бэкенд для этой же карты сам использует
    приоритет `prof.full_name or current_user.display_name` (см.
    `compatibility.py::composite_interpret`) — фронтенд этот приоритет не
    повторял и брал только `display_name`. Обычная карточка совместимости
    вообще не показывает "моё" имя в заголовке (только `result.partner_name`),
    поэтому сравнение с "Оксана" из ТЗ было не совсем яблоки-к-яблокам, но
    сама причина подтвердилась.
  - **Руна "Феху" другим цветом (Problem 4):** подтверждено — это
    осознанная фича, не баг. `rune.reversed` красит подпись руны в
    `#D98A8A` (обратная) вместо `#8A7FC0` (прямая) — Феху в конкретном
    раскладе была перевёрнута, остальные 4 — нет. На основной странице
    Рун это дублируется текстовым бейджем "Перевёрнутая"/"Прямая", а в
    самой share-карточке текстового пояснения не было — только цвет.
  - `"Composite Chart"` нигде в интерфейсе не был переведён на русский
    (ни в `typeLabels` на странице Совместимости, ни в самом
    `CompositeChart.tsx`) — использовал "Композитная карта" везде.
- **Сделано:**
  - `frontend/src/components/ShareCard.tsx`:
    - `type` union: добавлен `"composite"`. Убран мёртвый проп `sign`
      (использовался только багованным composite-вызовом).
    - Новые пропы: `natalName`, `bigThree` (массив `{label, sign, degree}`),
      `aspectLabel`, `description`.
    - Блок `natal`: имя (если есть) + 3 строки Солнце/Луна/Асцендент со
      знаком и градусом — как на самой странице.
    - Новый блок `composite`: главный аспект (`aspectLabel`) + текстовое
      описание (`description`).
    - Блок `runes`: добавлена легенда под рунами — если хоть одна
      перевёрнута, показывается строка-пояснение (что именно значит
      цвет), новый ключ `share.reversed_hint`.
  - `frontend/src/pages/NatalChart.tsx`: вычисляется `bigThreeForShare` из
    `chart.planets[0..1]` + `chart.ascendant` (тот же источник, что и
    видимый на странице блок "Большая тройка"); в `ShareCard` передаются
    `natalName={form.name}` и `bigThree`.
  - `frontend/src/components/CompositeChart.tsx`:
    - Добавлен fetch `/api/v1/profile` → `profileFullName`; `userName =
      profileFullName || user?.name || "?"` — тот же приоритет, что уже
      использует бэкенд для этой фичи.
    - `type="composite"`, `aspectLabel` — главный (наименьший орб) аспект
      композитной карты, `description` — обрезанный до 140 символов текст
      уже сгенерированной AI-интерпретации (если пользователь её открывал).
    - `subtitle`, "Планеты Composite" и "Расчёт composite chart..."
      переведены на "Композитная карта" / "Планеты композитной карты" /
      "Расчёт композитной карты...".
  - `frontend/src/pages/Compatibility.tsx`: `typeLabels.composite` —
    "Composite Chart" → "Композитная карта" (RU).
  - `frontend/src/i18n/locales/{ru,en,es,pt,tr,uk}.json`: добавлены
    `share.type_composite` и `share.reversed_hint` во все 6 языков.
- **Проверено:**
  - `tsc --noEmit` — 0 ошибок.
  - Визуально (моковые данные, временный harness, не закоммичен) — все
    7 типов карточек (`tarot`, `runes`, `numerology`, `natal`, `compat`,
    `composite`, `lunar`) отрендерены и проверены через DOM: натальная
    показывает имя + 3 строки Большой тройки с градусами; composite
    показывает оба имени, переведённый подзаголовок, главный аспект и
    описание; руны — Феху `rgb(217,138,138)` (перевёрнута), остальные 4
    `rgb(138,127,192)` (прямые), плюс легенда под рядом рун; остальные
    4 типа не сломались (регрессии нет).
- **Следующий шаг:**
  - Alembic миграции (вместо create_all)
  - Тестирование уведомлений на VPS
  - Имя PNG-файла при экспорте (сейчас всегда `mystral-reading.png`) —
    в бэклог, не блокирует

## 2026-07-05 — TZ-057 доп. фикс: сырой markdown в AI-тексте (`**bold**` виден как есть)

- **Найдено:** после фикса composite-карточки Саша заметил на скриншоте
  буквальные `**...**` в тексте AI-интерпретации внутри карточки. Причина
  системная, не только в ShareCard: `{interpretation}` (или `{aiText}`)
  рендерится как обычный React-текст без markdown-парсера в 6 местах —
  `Tarot.tsx`, `Compatibility.tsx`, `Runes.tsx`, `Numerology.tsx`,
  `NatalChart.tsx`, `CompositeChart.tsx` (плюс попадает в `ShareCard`
  через `description`). LLM (deepseek через OpenRouter) периодически
  сам генерирует markdown-разметку (`**жирный**`), хотя раньше в system
  prompt это явно не запрещалось.
- **Сделано (два уровня защиты):**
  - `frontend/src/utils/markdown.ts` — новая `stripMarkdown()`: убирает
    `**bold**`, `*italic*`, markdown-заголовки `#`, буллеты `- `,
    инлайн-код `` ` `` через regex (не полноценный парсер, просто
    зачистка спецсимволов, которые LLM иногда добавляет).
    Применена во всех 6 местах отображения AI-текста, а в
    `CompositeChart.tsx` — и в `shareDescription` (зачистка ДО обрезки
    до 140 символов, чтобы не резать посреди `**`).
  - `backend/app/core/groq_client.py` — в `_QUALITY_DIRECTIVE`
    (универсальный system prompt, добавляется ко всем роутерам) для
    всех 6 языков добавлена строка с явным запретом markdown-разметки
    (`**`, `*`, `#`, обратные кавычки, списки через дефис) — снижает
    частоту проблемы на источнике, не заменяя фронтовую зачистку.
- **Проверено:**
  - `stripMarkdown` протестирован на точном примере со скриншота
    (`**Солнце в Водолее (1.6°)**...`) — звёздочки убраны, текст читается
    нормально.
  - `tsc --noEmit` — 0 ошибок.
  - `python -m py_compile app/core/groq_client.py` — 0 ошибок.
  - `pytest` — 86 passed, 2 skipped (регрессий нет, backend-логика не
    менялась, только текст промпта).
- **Следующий шаг:**
  - Alembic миграции (вместо create_all)
  - Тестирование уведомлений на VPS
  - Имя PNG-файла при экспорте (сейчас всегда `mystral-reading.png`) —
    в бэклог, не блокирует

## 2026-07-05 — Фикс кнопок "Поделиться" (скачивание/отправка в Telegram)

- **Жалоба:** "Скачать PNG" работает неудобно, а кнопка "Telegram"
  говорит, что картинка скопирована, но вставить её в чат не получается.
- **Причина:** обе кнопки в `useShareCard.ts` делали одно и то же —
  пытались `navigator.share({files:[file]})`, и только если API
  недоступен, откатывались на скачивание. Внутри Telegram Mini App
  (in-app WebView) `navigator.share` для файлов обрабатывается самим
  Telegram нестабильно: может отрапортовать успех и показать "скопировано",
  но фактически передать что-то, что не вставляется как изображение.
  Ни разу не было прямого управления буфером обмена с нашей стороны —
  весь "copy" происходил где-то внутри чужого API.
- **Сделано:** `frontend/src/hooks/useShareCard.ts`:
  - `share()` ("Скачать PNG") — убран `navigator.share` полностью,
    теперь всегда прямое скачивание файла. Кнопка называется "скачать" —
    теперь она буквально просто скачивает, без обходного share-sheet.
  - `shareToTelegram()` ("Telegram") — теперь сама пишет PNG в буфер
    обмена через `navigator.clipboard.write([new ClipboardItem({"image/png": file})])`
    (мы сами гарантируем корректный MIME-тип, а не полагаемся на то, как
    Telegram под капотом обрабатывает `navigator.share`). При успехе —
    подсказка "Скопировано! Открой чат в Telegram и вставь изображение
    (Ctrl+V...)". Если Clipboard API недоступен — откат на
    `navigator.share` (работает в обычных браузерах вне Telegram), если и
    это недоступно — прямое скачивание с подсказкой "прикрепи вручную".
  - `ShareCard.tsx` — новое поле `feedback` из хука показывается под
    кнопками как текстовая подсказка.
  - Новые i18n-ключи `share.copied_hint` / `share.download_hint` во всех
    6 языках.
- **Проверено:**
  - `tsc --noEmit` — 0 ошибок.
  - Визуально (harness, не закоммичен): клик по "Telegram" — при
    недоступности Clipboard API (в этом окружении) корректно откатился
    на скачивание с подсказкой "Изображение сохранено — прикрепи его в
    Telegram вручную", ошибок в консоли нет; клик по "Скачать PNG" —
    кнопка возвращается в обычное состояние без зависаний.
  - На реальном устройстве (не проверено мной физически, т.к. нужен
    настоящий пользовательский клик — синтетические клики через JS не
    всегда дают "trusted" activation, нужный Clipboard API) ожидается,
    что `clipboard.write` сработает и вставка в Telegram будет работать
    напрямую — стоит перепроверить на проде после деплоя.
- **Следующий шаг:**
  - Alembic миграции (вместо create_all)
  - Тестирование уведомлений на VPS
  - Имя PNG-файла при экспорте (сейчас всегда `mystral-reading.png`) —
    в бэклог, не блокирует
  - Перепроверить `shareToTelegram` на реальном телефоне (в самом
    Telegram Mini App и в обычном мобильном браузере) после деплоя

## 2026-07-05 — ТЗ-038b: реальная интеграция ЮKassa (замена заглушки)

- **Найдено:** ТЗ предполагало, что вся интеграция ЮKassa — заглушка.
  На деле бэкенд (`payments.py`) уже делал реальный вызов создания
  платежа и реально верифицировал вебхук обратным запросом к API
  ЮKassa (сделано ранее в рамках QA-аудита). Настоящая заглушка была
  во фронтенде: `PaywallSheet.tsx` для НЕ-Telegram пользователей просто
  показывала "напишите нам на почту", ни разу не обращаясь к
  существующему эндпоинту. Помимо этого — платежи нигде не сохранялись
  в БД, вебхук игнорировал `payment.canceled`, `return_url` вёл в
  Telegram, а не на страницу результата, и не было обработки возврата
  пользователя после оплаты.
- **Сделано (бэкенд):**
  - `app/models/user.py` — новая таблица `Payment` (id, user_id,
    provider, provider_payment_id, product, amount, currency, status,
    created_at/updated_at); экспортирована из `app/models/__init__.py`
    (миграция не нужна — `create_all()` подхватит при рестарте).
  - `config.py` — новый `frontend_url` (return_url больше не ведёт в
    Telegram; `telegram_webapp_url` не переиспользован — он занят
    другим уведомлением в `scheduler.py`).
  - `yukassa_create` — теперь создаёт свою `Payment(status="pending")`
    ДО вызова ЮKassa (id платежа генерируется у нас, т.к. id из ответа
    ЮKassa физически не может попасть в `return_url` того же запроса,
    который этот ответ produces), после ответа ЮKassa — заполняет
    `provider_payment_id` и коммитит. Обёрнуто в try/except на
    `httpx.HTTPError` → 502 вместо необработанного исключения.
  - Новый общий helper `_verify_and_sync_payment(payment_id, session)`
    — переиспользуется вебхуком и статус-эндпоинтом: обратный GET-запрос
    к API ЮKassa (never trust webhook body/params alone), синхронизация
    локальной строки, активация Pro строго один раз на платёж (guard
    `was_succeeded` — иначе повторная доставка вебхука от ЮKassa
    бесплатно продлевала бы `subscription_expires_at` при каждом ретрае,
    реальный денежный баг).
  - `yukassa_webhook` — теперь принимает `payment.succeeded` И
    `payment.canceled` (раньше `canceled` молча игнорировался), вся
    логика верификации — через общий helper.
  - Новый `GET /payments/yukassa/status/{payment_id}` — статус по
    НАШЕМУ id платежа (из `return_url`), с проверкой владельца (403 на
    чужой платёж), при `status="pending"` — живая доверификация через
    тот же helper (не ждать вебхук, если пользователь уже вернулся на
    страницу).
  - Секретный ключ ЮKassa нигде не логируется и не попадает в ответы
    API — используется только внутри `base64.b64encode(...)` для
    заголовка `Authorization`; все `HTTPException.detail` — статичные
    строки, сырой JSON от ЮKassa клиенту не возвращается.
- **Сделано (фронтенд):**
  - `PaywallSheet.tsx` — `handleBuy()` для не-Telegram пользователей
    теперь реально вызывает `/payments/yukassa/create` и делает
    `window.location.href = data.payment_url`; убран мёртвый код
    заглушки (`showYukassaStub` state, сброс в `PlanCard`, JSX-блок
    "напишите нам").
  - `utils/api.ts` — новый `apiGet<T>()` (GET-версия существующего
    `apiRequest`, тот же паттерн парсинга ошибок).
  - Новая страница `pages/PaymentReturn.tsx` — читает `payment_id` из
    URL, поллит статус-эндпоинт (до 5 раз, 2с пауза — тот же ритм, что
    уже использует Stars-флоу в `PaywallSheet`), показывает
    успех/отмену/pending-таймаут, при успехе — `updateUser({tier:"pro"})`
    и очистка URL.
  - `App.tsx` — `if (window.location.search.includes("payment_id=")) return <PaymentReturn />;`
    добавлено ПОСЛЕ проверки `!user` (в отличие от `ResetPassword`,
    возврат с оплаты требует авторизации).
- **Тесты:** новый класс `TestYukassaFlow` в `test_payments.py` (11
  кейсов) — создание платежа и персист строки, вебхук
  succeeded/canceled/неизвестное событие, **регрессионный тест на
  повторную доставку вебхука** (проверяет, что `subscription_expires_at`
  не сдвигается второй раз), статус-эндпоинт (401/403/404, живая
  доверификация pending). При мокании `httpx.AsyncClient.post/.get`
  словил тонкий баг: `conftest.py`'s `client`-фикстура сама является
  `httpx.AsyncClient` (ASGITransport) — бланкет-патч класса перехватывал
  и собственные запросы теста к приложению, а не только исходящий вызов
  к ЮKassa. Исправлено URL-дискриминирующей обёрткой (фейковый ответ
  только если домен — `api.yookassa.ru`, иначе — реальный метод,
  захваченный до патча).
- **Проверено:**
  - `tsc --noEmit` — 0 ошибок.
  - `pytest` — 97 passed, 2 skipped (2 skip — те же, что раньше,
    `pyswisseph` недоступен на Windows вне Docker; к этому тикету не
    относится, платежи не используют kerykeion).
- **Не входит / отложено:**
  - Rate-limit на `/yukassa/webhook` — публичный, без авторизации,
    сейчас без лимита (как и раньше). Не блокирует критерии готовности.
  - Тестирование через тестовый магазин ЮKassa / реальная оплата —
    Саша решил разобраться с этим сам после деплоя.
  - **Ручной шаг для Саши после деплоя:** в личном кабинете ЮKassa
    указать webhook URL — `https://mystral.space/api/v1/payments/yukassa/webhook`
    (или актуальный домен) для событий `payment.succeeded` и
    `payment.canceled`. Без этого шага активация Pro всё ещё сработает
    (статус-эндпоинт делает живую доверификацию при возврате
    пользователя на страницу), но с задержкой до захода пользователя на
    `PaymentReturn`, а не мгновенно по вебхуку.
- **Следующий шаг:**
  - Alembic миграции (вместо create_all)
  - Тестирование уведомлений на VPS
  - Перепроверить `shareToTelegram` на реальном телефоне после деплоя
  - Настроить webhook URL ЮKassa в личном кабинете (см. выше)

## 2026-07-05 — ТЗ-058: фикс бинарных байтов в regex (api.ts)

- **Найдено:** во время работы над ТЗ-038b обнаружилось, что
  `frontend/src/utils/api.ts` содержит буквальные управляющие байты
  (сырой NUL `0x00` и DEL `0x7F`) прямо внутри исходников — внутри
  regex-паттерна `cleanChunk` (`[^\x00-\x7f...]`), а не текстовые
  escape-последовательности. Скорее всего попало туда ещё во время
  более раннего фикса `clean_chunk` (первый тикет в этой сессии).
  Поведение это не меняло (JS парсит сырой control-байт в regex-литерале
  идентично экранированной записи `\x00`), но `git diff` показывал
  "Binary files differ" вместо обычного текстового diff — читать/ревьюить
  файл было невозможно.
- **Сделано:** оба байта (`0x00` и `0x7F`) заменены на буквальный текст
  `\x00` / `\x7f` (4 ASCII-символа каждый: `\`, `x`, `0`, `0`). Правка
  сделана на уровне байтов через отдельный Python-скрипт (не через
  обычный текстовый редактор — сырые control-байты не всегда надёжно
  проходят через обычные текстовые тулы/шелл-экранирование).
- **Проверено:**
  - Побайтовое сравнение старой (git HEAD) и новой версии файла: после
    обратной подстановки `\x00`/`\x7f` (текст) → сырые байты содержимое
    полностью идентично — изменилось только представление, не сам текст.
  - Прямое исполнение regex в Node.js — извлечён `cleanChunk` regex из
    старой (HEAD) и новой версии файла, прогнан на 8 репрезентативных
    строках (кириллица, градусы/№/%, знаки зодиака ♈-♓, "случайная
    латиница" типа café/naïve, control-символы, CJK-символы, markdown-
    мусор, пустая строка, чистый ASCII) — результат идентичен байт в
    байт для каждой строки в обеих версиях.
  - `tsc --noEmit` — 0 ошибок.
  - Юнит-тестов на фронтенде в проекте нет вообще (test runner не
    настроен) — ручная проверка через Node сыграла роль регрессионного
    теста для этой конкретной функции.
  - `git diff` для этого коммита всё ещё показывает "Binary files
    differ" — ожидаемо и неизбежно для самого переходного коммита,
    т.к. старая сторона (HEAD) физически содержит настоящий NUL-байт;
    начиная со следующего коммита, затрагивающего этот файл, diff будет
    обычным текстовым.
- **Следующий шаг:**
  - Alembic миграции (вместо create_all)
  - Тестирование уведомлений на VPS
  - Перепроверить `shareToTelegram` на реальном телефоне после деплоя
  - Настроить webhook URL ЮKassa в личном кабинете

## 2026-07-06 — ТЗ-037c: SEO-страницы на 5 языках (615 страниц)

- **Контекст/находки:**
  - Языки: en, es, pt, tr, uk (код украинского — `uk`, не `ua` — так во
    всём проекте). 123 страницы × 5 языков = 615 новых + 3 хаба на язык.
  - Таблица `SeoContent` УЖЕ имела колонку `lang` и индекс
    `(page_type, slug, lang)` — мультиязычность была заложена, но
    `seo_generator.py` хардкодил `lang=="ru"` в SELECT и INSERT.
  - Готовых переводов контента нет (i18n — только UI-строки), поэтому
    контент генерируется AI, как и русский — через тот же пайплайн.
  - Попутные баги, закрытые этим тикетом: (1) `_CLEAN_RE` вырезал
    турецкие буквы ğ/ş/ı/İ (Latin Extended-A вне разрешённых диапазонов);
    (2) существование строки в `warm_seo_cache` проверялось без фильтра
    по lang; (3) JSON-LD BreadcrumbList нумерологии ссылался на
    несуществующий хаб `/numerology` (404) — сокращён до 2 крошек.
- **Сделано (бэкенд):**
  - `app/data/seo_i18n.py` (новый) — статические переводы: UI-хром
    (~42 ключа × 6 языков), знаки зодиака (имена/даты/стихии/камни/цвета,
    12 × 5), Старшие Арканы (22 × 5), ранги/масти + пер-языковый формат
    имён Младших Арканов (турецкий — масть первой с притяжательным
    аффиксом: «Asa İkilisi»), руны (латиница для en/es/pt/tr, кириллица
    для uk), архетипы нумерологии; хелперы `localize_*`, `abs_url`,
    `hreflang_alternates` (7 пар: 6 языков + x-default → русская версия).
  - `seo_generator.py` — `get_seo_content(..., lang)`; для ru — прежние
    промпты байт-в-байт, для 5 новых — английские мастер-промпты с
    явной директивой целевого языка (данные передаются уже
    локализованными); **stale-while-revalidate**: протухшая строка
    отдаётся сразу, обновление — фоновой задачей (краулеры больше не
    ждут 10–20с на регенерации); пер-языковые fallback'и с маркером
    `_fallback` → шаблон ставит `noindex, follow` (пустышки не
    индексируются, автоматически откроются после генерации контента);
    `warm_seo_cache` — только языки из `settings.seo_warm_langs`
    (default "ru" — деплой не запускает часы фоновой генерации).
  - `seo_pages.py` — каждый хендлер получил второй декоратор
    `/{lang}/...` (язык читается из `request.path_params`, НЕ из
    сигнатуры — иначе FastAPI биндит его из query-string на корневых
    роутах, что и было первой версией бага в тестах); `?lang=xx` из
    старой разметки → 301 на `/{xx}/...`; canonical — self-referencing
    на каждой версии; sitemap.xml — 757 `<loc>` (126 путей × 6 языков +
    главная) с `xhtml:link` hreflang-аннотациями по спецификации.
  - Шаблоны (base + 7) — `<html lang>`, hreflang-цикл (заменил мёртвые
    `?lang=`-ссылки и добавил отсутствовавший en), весь хром из
    словаря переводов, переключатель языков обычными `<a href>` под
    крошками, JSON-LD с `inLanguage` и переведёнными крошками,
    внутренние ссылки с префиксом языка.
  - `scripts/generate_seo_translations.py` (новый) — батч-генерация:
    `--langs/--types/--slugs/--force/--delay/--dry-run`, resume
    (пропускает существующие строки), экспоненциальный backoff, сводка
    неудач, pre-flight проверка `OPENROUTER_API_KEY`, отклоняет `ua`
    с подсказкой про `uk`.
  - `nginx/nginx.prod.conf` — regex SEO-локации расширен префиксом
    `((en|es|pt|tr|uk)/)?`; порядок локаций сохранён (статика по
    расширениям — раньше, `/tarot/5.jpg` по-прежнему уходит на
    frontend). Синтаксис проверен `nginx -t` в docker.
- **Проверено:**
  - `pytest` — **125 passed, 2 skipped** (29 новых SEO-тестов: языковые
    страницы/хабы/SVG, валидация языка (404 для `/de/...`), 301-редиректы
    `?lang=`, RU-регрессия (canonical/title/крошки байт-в-байт, включая
    отсутствие `?lang=` и `/ru/`-ссылок), sitemap ровно 757 loc +
    well-formed XML, noindex на fallback).
  - SSR-чек через ASGI (эквивалент curl): `/es/zodiac/aries`,
    `/tr/tarot/the-fool`, `/uk/runes/fehu`, `/en/numerology/life-path-7`,
    `/pt/zodiac` — правильный `<html lang>`, локализованные title/h1,
    self-canonical, 7 hreflang, переключатель со всеми 6 языками.
  - `tsc --noEmit` — 0 ошибок (фронтенд не затронут).
- **Ручные шаги для Саши после деплоя:**
  1. **Запустить батч-генерацию контента на проде** (~1.5–2 часа на все
     615 страниц, стоимость порядка нескольких $ OpenRouter):
     `docker compose exec backend python scripts/generate_seo_translations.py`
     (можно частями: `--langs es`, `--types zodiac` и т.д.; прерванный
     запуск продолжается с места остановки). До генерации страницы
     отдают короткую заглушку с `noindex` — не индексируются.
  2. Выборочно проверить контент: `curl https://mystral.space/es/zodiac/aries`
     и 3–5 других страниц на языках — текст должен быть на целевом языке.
  3. Отправить обновлённый sitemap.xml в Яндекс.Вебмастер и Google
     Search Console.
- **Следующий шаг:**
  - Alembic миграции (вместо create_all)
  - Тестирование уведомлений на VPS
  - Перепроверить `shareToTelegram` на реальном телефоне после деплоя
  - Настроить webhook URL ЮKassa в личном кабинете
  - Честный per-page lastmod в sitemap (по `generated_at`) — бэклог
  - Уникальный индекс `(page_type, slug, lang)` от гоночных дублей — бэклог

## 2026-07-06 — ТЗ-060: систематическая ошибка парсинга JSON при генерации SEO

- **Найдено:** при запуске `generate_seo_translations.py` на проде —
  повторяющаяся ошибка `Expecting ',' delimiter` при разборе JSON-ответа
  DeepSeek V4 Flash, на разных страницах и языках. Корень — модель в
  свободной генерации не всегда экранирует спецсимволы (кавычки) внутри
  строковых значений JSON; полагаться на "аккуратность" модели ненадёжно
  в масштабе 615 страниц.
  - Проверил через OpenRouter API (`/api/v1/models/.../endpoints`):
    `deepseek/deepseek-v4-flash` поддерживает `response_format` и
    `structured_outputs` практически на всех провайдерах маршрутизации
    (DeepInfra, Baidu, StreamLake, Alibaba, Fireworks и др.) — значит
    проблему можно закрыть на уровне API, а не только промпт-инженерией.
- **Сделано (три уровня защиты):**
  1. `seo_generator.py`: `response_format={"type": "json_object"}` в
     вызове `chat.completions.create` — форсирует синтаксически валидный
     JSON на уровне API (grammar-constrained decoding), а не только по
     просьбе в промпте. Это корневой фикс.
  2. Новый `_parse_content_json()`: при сбое `json.loads` — fallback на
     `json_repair.loads()` (новая зависимость `json-repair` в
     `requirements.txt`). **Важно:** `json_repair` на безнадёжном мусоре
     не бросает исключение, а молча возвращает `''` — поэтому результат
     repair'а всегда валидируется на `dict` с обязательными ключами
     (`intro/sections/faq/cta_text`) перед тем как считаться успехом;
     иначе — `None` и явный `logger.error(...)` с обрезанным сырым
     ответом для диагностики.
  3. Промпты (`_QUALITY`/`_QUALITY_I18N`, все 6 языков) — явная инструкция
     не использовать прямую двойную кавычку внутри значений JSON-строк
     (использовать «ёлочки»/одинарные), с примером корректного значения;
     отдельно уточнено, что апострофы (it's, don't) экранировать не
     нужно — это не то же самое, что кавычка, специфично для английского,
     откуда и пошла изначальная ошибка.
  - Попутно закрыта тихая потеря контента: ветка "фигурные скобки не
    найдены" раньше возвращала fallback без единой строки в логах —
    теперь логируется наравне с остальными путями отказа.
- **Проверено:**
  - Новый `tests/test_seo_generator.py` (10 тестов, без сети): точное
    воспроизведение прод-ошибки — JSON с буквальной незаэкранированной
    кавычкой внутри строки даёт `json.JSONDecodeError: Expecting ','
    delimiter` при обычном `json.loads`, и корректно восстанавливается
    через `_parse_content_json` (текст `"ram"` внутри intro сохранён);
    JSON, обёрнутый моделью в markdown-код-блок; полностью нечитаемый
    мусор → `None`, не мусорный dict; отсутствие обязательных ключей →
    `None`; промпты форматируются без `KeyError`/лишних фигурных скобок
    для всех 4 типов страниц × всех 6 языков (регрессия на мою же правку
    `_QUALITY_I18N` — там нужно было задвоить `{{`/`}}` в JSON-примере).
  - Полный `pytest` — **135 passed, 2 skipped** (регрессий нет).
  - `tsc --noEmit` — 0 ошибок (фронтенд не затронут).
  - Живой вызов OpenRouter с реальным ключом не проверялся из песочницы
    (нет доступа к прод-окружению/ключу) — критерий готовности "прогон
    на 10–15 страницах, 0 failed по JSON" нужно проверить на самом
    сервере перед массовым перезапуском (см. ручной шаг ниже).
- **Ручной шаг для Саши:**
  1. Остановленный ранее скрипт (Ctrl+C) можно спокойно перезапускать —
     `--force` не передавался, значит уже сгенерированные страницы не
     потеряны и не перегенерируются повторно.
  2. Сначала — пробный прогон на малом наборе страниц, где раньше падало
     (en и tr упоминались явно):
     `docker compose exec backend python scripts/generate_seo_translations.py --langs en,tr --types zodiac --slugs aries,taurus,gemini`
     Ожидание: 0 failed по причине JSON-парсинга (в логе — при сбое
     строгого парсинга будет строка "trying json_repair", а не сразу
     FAILED; полный отказ будет только на реальных сетевых сбоях/рейт-
     лимитах, не на форматировании).
  3. Если пробный прогон чистый — можно перезапускать оставшиеся языки
     без `--force` (resume подхватит прогресс):
     `docker compose exec backend python scripts/generate_seo_translations.py`
- **Следующий шаг:**
  - Alembic миграции (вместо create_all)
  - Тестирование уведомлений на VPS
  - Перепроверить `shareToTelegram` на реальном телефоне после деплоя
  - Настроить webhook URL ЮKassa в личном кабинете
  - Честный per-page lastmod в sitemap (по `generated_at`) — бэклог
  - Уникальный индекс `(page_type, slug, lang)` от гоночных дублей — бэклог
  - После завершения батч-генерации — выборочная проверка контента и
    отправка sitemap в Яндекс.Вебмастер/Google Search Console (ТЗ-037c)

## 2026-07-06 — ТЗ-060 доп. фикс: обрезка ответа по max_tokens

- **Найдено на проде (после редеплоя фикса ТЗ-060):** лог показал, что
  `response_format=json_object` + `json_repair` реально заработали
  (`trying json_repair` в логе — подтверждение, что задеплоился новый
  код), НО `json_repair` вернул синтаксически валидный, но **неполный**
  результат — есть `intro` и все 10 `sections`, но нет `faq`/`cta_text`.
  Причина видна прямо в восстановленном тексте секции "Famous Aries
  People": модель рассуждала вслух и сама себя поправляла («Wait, he's
  Aquarius... Actually, let me correct... Actually, let's list
  accurate...») — это съело весь бюджет `max_tokens=3000`, и генерация
  оборвалась до того, как модель дошла до `faq`/`cta_text`. Моя же
  валидация обязательных ключей сработала верно — не приняла неполный
  результат, ушла в retry (что и видно в логе), но с тем же промптом и
  тем же `max_tokens` повтор рисковал воспроизводить ту же обрезку.
- **Сделано:**
  - `max_tokens` вынесен в именованную константу `GENERATION_MAX_TOKENS`
    и поднят с 3000 до 4096 — проверил через OpenRouter API лимиты
    вывода по всем провайдерам `deepseek/deepseek-v4-flash` (минимум
    16 384 токенов у самого скромного — DeepInfra), так что упереться в
    лимит провайдера с 4096 невозможно, узкое место было только в
    собственной настройке.
  - `_QUALITY`/`_QUALITY_I18N` (все 6 языков) — новая инструкция: писать
    уверенно и окончательно, никогда не «рассуждать вслух» и не
    поправлять себя в тексте («wait, actually» / «на самом деле» и т.п.);
    в пункте про известных людей — ровно 3–4 реальных имени с одной
    фразой каждое, без обсуждения точности дат рождения. Это убирает
    сам источник раздувания токен-бюджета, а не только чинит его симптом.
- **Проверено:**
  - Новые тесты в `test_seo_generator.py` (13 всего, было 10): точное
    воспроизведение обнаруженной формы отказа — валидный JSON с
    `intro`+`sections`, но без `faq`/`cta_text` (ровно то, что вернул
    `json_repair` на проде) — `_parse_content_json` корректно отклоняет
    (`None`), не проглатывает как валидный контент; регрессионный тест
    на `GENERATION_MAX_TOKENS >= 4096`; проверка, что новые директивы
    против «рассуждений вслух» и лимит в 3–4 имени присутствуют в
    промптах обеих языковых веток.
  - Полный `pytest` — **138 passed, 2 skipped**.
- **Ручной шаг для Саши:** снова `git pull && ./deploy.sh` (или
  `docker compose -f docker-compose.prod.yml up --build -d backend`),
  затем тот же пробный прогон:
  `docker compose exec backend python scripts/generate_seo_translations.py --langs en,tr --types zodiac --slugs aries,taurus,gemini`
  Если и в этот раз где-то не хватит токенов — в логе будет видно
  ("invalid/incomplete result") без порчи данных; если пробный прогон
  чистый — перезапускать оставшиеся языки без `--force` (resume).
- **Также поправлено:** корневой `deploy.sh` использовал устаревший
  `docker-compose` (дефис, v1) вместо `docker compose` (плагин v2,
  установлен на сервере) — падал на шаге сборки образа. `scripts/deploy.sh`
  такой проблемы не имел (уже использовал `docker compose`).
- **Следующий шаг:**
  - Alembic миграции (вместо create_all)
  - Тестирование уведомлений на VPS
  - Перепроверить `shareToTelegram` на реальном телефоне после деплоя
  - Настроить webhook URL ЮKassa в личном кабинете
  - Честный per-page lastmod в sitemap (по `generated_at`) — бэклог
  - Уникальный индекс `(page_type, slug, lang)` от гоночных дублей — бэклог
  - После завершения батч-генерации — выборочная проверка контента и
    отправка sitemap в Яндекс.Вебмастер/Google Search Console (ТЗ-037c)


---

## 2026-07-06 — ТЗ-060 доп. фикс #2: тот же обрыв, но уже без «раздумий вслух»

### Completed
- Прод после фикса #1: снова `Expecting ',' delimiter`, но контент чистый (intro+10 секций+5 FAQ, без саморассуждений — анти-rambling сработала). Обрыв на последнем поле `cta_text`.
- Причина неизвестности: `finish_reason` от API не логировался — не было прямого сигнала "обрыв по лимиту токенов vs другая ошибка".
- `GENERATION_MAX_TOKENS`: 4096 → **6144**.
- Логирование `finish_reason`: если `"length"` — WARNING до попытки парсинга.

### Problems found
- tiktoken-оценка (cl100k_base, прокси для токенизатора DeepSeek) легла в диапазон, где 4096 могло не хватить впритык — цифра ненадёжна, см. фикс #3.

### Verified
- Новый тест в `test_seo_generator.py` (13→14): мок `finish_reason="length"` + обрезанный JSON — проверяет лог и контролируемый fallback. Полный `pytest` — 139 passed, 2 skipped.
- Прод-прогон `--langs en,tr --types zodiac --slugs aries,taurus,gemini`: **3 generated, 3 skipped, 0 failed**. ТЗ-060 закрыто (на тот момент) — можно запускать оставшиеся ~600 страниц.

### Next step
- `git pull && ./deploy.sh`. Если `Expecting ',' delimiter` вернётся: `finish_reason=length` в логе → токен-бюджет (поднять лимит ещё); без него → другой класс ошибки.

---

## 2026-07-06 — ТЗ-060 доп. фикс #3: 6144 тоже не хватило

### Completed
- Полный прогон 615 страниц: после 6 успешных en-страниц — `zodiac/capricorn/en` обрезан, `finish_reason=length` в логе подтвердил причину (сигнал из фикса #2 сработал).
- `GENERATION_MAX_TOKENS`: 6144 → **12000** (сразу щедрый запас, не по чуть-чуть). `max_tokens` — потолок, не стоимость: платится по факту сгенерированного.

### Problems found
- tiktoken-оценка по смещению обрыва дала ~3–4.5k токенов — противоречит факту обрыва при лимите 6144. Вывод: cl100k_base недостаточно точно аппроксимирует токенизатор DeepSeek для таких расчётов — дальше полагаться на эмпирику (реальные обрывы), не на вычисления.

### Verified
- Регрессионный тест на порог `GENERATION_MAX_TOKENS >= 12000`. Полный `pytest` — 139 passed, 2 skipped.

### Next step
- `git pull && ./deploy.sh`, продолжить полный прогон (resume пропустит уже успешные страницы): `docker compose exec backend python scripts/generate_seo_translations.py`. Обрыв с `finish_reason=length` → лимита всё ещё не хватает, прислать лог; без него → другой класс проблемы.

---

## 2026-07-07 — ТЗ-037c/ТЗ-060: полная батч-генерация завершена

### Completed
- Прод: **577 generated, 38 skipped, 0 failed** — все 615 страниц (123 × 5 языков), ноль сбоев парсинга JSON. 38 skipped — уже сгенерированные в пробных прогонах, resume отработал корректно.
- Все три уровня фиксов ТЗ-060 (`response_format=json_object`, прогрессия `GENERATION_MAX_TOKENS` 3000→4096→6144→12000, анти-rambling промпт, логирование `finish_reason`) в сумме дали 0% отказов на полном объёме.
- ТЗ-037c закрыто по контенту.

### Next step
- Ручное (вне доступа Claude Code): проверить несколько страниц curl'ом (`/es/zodiac/aries`, `/tr/tarot/the-fool`, `/uk/runes/fehu`) на язык контента; отправить обновлённый `sitemap.xml` в Яндекс.Вебмастер и Google Search Console.
- Alembic миграции (вместо create_all); тестирование уведомлений на VPS; перепроверить `shareToTelegram` на телефоне; webhook URL ЮKassa в личном кабинете; честный per-page lastmod в sitemap — бэклог; уникальный индекс `(page_type, slug, lang)` от гоночных дублей — бэклог.

---

## 2026-07-07 — ТЗ-061: возврат средств не отзывал Pro-статус (критический баг)

### Completed
- Найдено: ТЗ-038b обрабатывал `payment.succeeded`/`payment.canceled`, но не `refund.succeeded`. Подтверждено на тесте: оплата → Pro → возврат → Pro остался активным бессрочно. Финансовая дыра.
- API ЮKassa: объект `refund` содержит `id` (ID возврата, НЕ платежа), `status`, `payment_id`, `amount`; вебхук — та же обёртка `{"type":"notification","event":"refund.succeeded","object":{...}}`. Частичные возвраты существуют (от 1₽).
- Решение (согласовано с Сашей): отзывать Pro при любом возврате (полном или частичном), без сравнения суммы — проще и безопаснее для бизнеса.
- `backend/app/api/v1/payments.py`:
  - `_revoke_pro(user)` — зеркалит `POST /admin/users/{id}/revoke-pro` (`tier="free"`, `expires_at=None`).
  - `_verify_and_revoke_refund(refund_id, session)` — обратный `GET /v3/refunds/{id}` к API ЮKassa (не доверяем телу вебхука), находит `Payment` по `payment_id`, отзывает Pro. Идемпотентно (уже `status="refunded"` → пропуск).
  - `yukassa_webhook` — принимает `refund.succeeded`; `object.id` для `refund.*` — ID возврата, не платежа (payment_id внутри объекта возврата).
  - `Payment.status` — добавлено значение `refunded` в комментарий (миграция не нужна — поле `str`, схема через `create_all`).

### Verified
- `TestYukassaRefund` (6 тестов): полный цикл оплата→Pro→возврат→отозван; частичный возврат тоже отзывает; идемпотентность повтора; возврат по неизвестному платежу → 400 без краша; тело вебхука врёт о статусе → переверификация не отзывает Pro; отсутствие `object.id` → `no_refund_id` без краша.
- Полный `pytest` — **145 passed, 2 skipped** (было 139, +6, без регрессий). `tsc --noEmit` — 0 ошибок.

### Next step
- Ручное (вне доступа — личный кабинет ЮKassa и прод-БД):
  1. Включить подписку на `refund.succeeded` в личном кабинете ЮKassa (Интеграция → HTTP-уведомления).
  2. Вручную снять Pro со своего тестового аккаунта (`POST /admin/users/{user_id}/revoke-pro` с admin-токеном) — старый вебхук повторно не придёт.
- Alembic миграции; тестирование уведомлений на VPS; перепроверить `shareToTelegram`; webhook URL ЮKassa; per-page lastmod в sitemap — бэклог; уникальный индекс `(page_type, slug, lang)` — бэклог.

---

## 2026-07-08 — Полный аудит проекта + фикс: ежедневный гороскоп в Telegram обрывался на полуслове

### Completed
- Полный функциональный аудит (8 параллельных read-only агентов, каждая находка перепроверена по коду, не по ТЗ/PROGRESS.md). Полный отчёт: `docs/archive/AUDIT_2026-07-07.md` (не читать целиком без необходимости — точечно смотреть нужный раздел). Топ-находки: реферальная программа не работает с момента внедрения (`RegisterRequest` не принимает `ref_code`); возврат Telegram Stars не отзывает Pro (тот же класс бага, что ТЗ-061 для ЮKassa); `/natal/calculate` и `/natal/svg` без авторизации и rate limit; push-уведомления никто не вызывает (только ручной self-test); структурные данные AI-фич (карты/руны/планеты/знаки) только ru/en; Privacy.tsx и Terms.tsx противоречат друг другу по сроку хранения данных (12 мес vs 30 дней — код подтверждает 30).
- Найдено по скриншоту (push "Гороскоп на сегодня"): текст обрывался на середине предложения. Причина — `app/services/horoscope.py:35` (`generate_horoscope`, только для планировщика Telegram-рассылки; интерактивный экран в приложении использует отдельный путь `max_tokens=700`, `horoscope.py:107`, не затронут) — `max_tokens=200` мало для 60-70 слов на кириллице. Тот же класс бага, что 3 раза чинили в ТЗ-060.
- `GENERATION_MAX_TOKENS` вынесен в константу, 200 → **500**. Логирование `finish_reason == "length"`.

### Verified
- `tests/test_horoscope_service.py` (3 теста): регрессия на порог `>= 500`, мок обрезанного/полного ответа. Полный `pytest` — **148 passed, 2 skipped** (было 145, без регрессий).

### Next step
- Alembic миграции (вместо create_all); перепроверить `shareToTelegram` на телефоне; webhook URL ЮKassa; per-page lastmod в sitemap — бэклог; уникальный индекс `(page_type, slug, lang)` — бэклог.
- Приоритизировать находки `AUDIT_2026-07-07.md` в отдельные тикеты: реферальная программа, возврат Stars, `/natal` без auth+rate limit, push, языковой пробел, противоречие Privacy/Terms.
- Подписка на `refund.succeeded` + ручное снятие Pro со своего аккаунта (см. ТЗ-061 выше).

---

## 2026-07-11 — ТЗ-062: фикс UUID в composite_interpret (500 на каждый вызов)

### Completed
- Диагностика (найдено в `docs/archive/AUDIT_2026-07-07.md`, подтверждено эмпирически по запросу Саши) — `compatibility.py:603`: `session.get(UserPartner, req.partner_id)` передавал str вместо UUID. Падало со `StatementError`/`AttributeError: 'str' object has no attribute 'hex'` на каждый вызов, для любого пользователя и раздела — весь AI-интерпретатор Composite Chart (overview/planets/aspects/advice) был нерабочим. Сам расчёт Composite Chart (не интерпретация, `compat_composite`) не затронут — там уже стоял `UUID(req.partner_id)`.
- Добавлен общий `_parse_partner_id(partner_id: str) -> UUID` в `compatibility.py`: оборачивает `UUID()`, превращает `ValueError` в `HTTPException(422, "Invalid partner_id")`. Ни одно из 4 мест разбора `partner_id` в файле раньше не обрабатывало невалидный формат (свалились бы в общий 500) — применено единообразно во всех четырёх местах (строки 208, 228, 526, 603), не только в багованном.

### Verified
- Новый `tests/test_compatibility.py`: невалидный формат partner_id → 422, не 500; partner_id чужого партнёра → 404, не 500. Оба гоняются без Docker.
- По одному тесту на все 4 раздела (overview/planets/aspects/advice) с валидным partner_id — требуют реального kerykeion/pyswisseph (нет Windows-колеса), помечены `skipif` по образцу `test_natal_timezone.py`: локально скипаются, реально прогоняются в Docker/CI.
- Полный `pytest` — **150 passed, 6 skipped** (было 148/2 — +2 passed, +4 skipped, без регрессий).
- `tsc --noEmit` — 0 ошибок. Фронтенд не менялся: `streamRequest`/`parseApiError` уже показывают `body.detail` для любого не-2xx статуса вне 401/402/429/5xx — новый 422 подхватывается существующей веткой без правок. `partner_id` на фронте всегда берётся из списка партнёров (реальный UUID), так что 422 — защитный случай на бэкенде, не реальный пользовательский сценарий.

### Next step
- Прогнать 4 skipped-теста реально в Docker (`docker compose exec backend pytest tests/test_compatibility.py -v`) на сервере — Саша делает деплой/pull сам.
- Alembic миграции; перепроверить `shareToTelegram`; webhook URL ЮKassa; per-page lastmod в sitemap; уникальный индекс `(page_type, slug, lang)` — бэклог.
- Остальные находки `AUDIT_2026-07-07.md`: реферальная программа, возврат Stars, `/natal` без auth+rate limit, push, языковой пробел, Privacy/Terms — в отдельные тикеты.

---

## 2026-07-11 — ТЗ-063: реферальная программа не начисляла бонусы никогда

### Completed
- Диагностика (отдельным заходом, до фикса): бэкенд-логика начисления (`POST /referrals/apply`, `_extend_pro`, `ReferralLog`) уже была полностью реализована и покрыта 5 тестами — разрыв был только в интеграции. `RegisterRequest` не принимал `ref_code`; фронтенд ни разу не вызывал `/referrals/apply`; Telegram-пути (auto-login/widget/merge) вообще не касались реферального кода; `startapp`/`start_param` deep-link не читался совсем (`initDataUnsafe` даже не был в типах).
- Решение: не трогать `RegisterRequest`/`register()` — вместо этого фронтенд сам вызывает уже готовый и протестированный `/referrals/apply` в момент, когда у нового пользователя впервые появляется JWT (email — сразу после `/auth/verify-email`; Telegram — сразу после `/auth/telegram` при `is_new === true`, во всех трёх местах, где дергается этот эндпоинт: auto-login, widget-login, и теперь единообразно).
- `backend/app/api/v1/referrals.py`: rate-limit на `/referrals/apply` (переиспользован `_check_ip_rate`/`_get_ip` из `auth.py`, тот же лимит 5/час, что у `register`); гейт `email_verified` для email-аккаунтов — с явным исключением для Telegram-нативных (`email is None`), иначе Telegram-бонусы никогда бы не начислялись (у них `email_verified` по умолчанию `False` и никогда не станет `True`); коллизия `ref_code` теперь исключена ретраем в `_ensure_ref_code` (колонка не была `unique`).
- Frontend: `utils/telegram.ts` — тип `initDataUnsafe.start_param`; `main.tsx` — мост `start_param` → тот же `localStorage["mystral_ref_code"]`, что и у веб-пути `/ref/CODE`, один механизм применения на оба источника; новый `applyStoredReferralCode()` в `utils/api.ts`; подключён в `VerifyEmail.tsx`, `AuthContext.tsx` (auto-login), `LoginScreen.tsx` (widget-login); убрано мёртвое поле `ref_code` из тела `/auth/register` (бэкенд его больше не читает и не будет). `Profile.tsx` — явная строка `total_bonus_days`.

### Verified
- Новый `TestReferralFullCycle` в `test_profile.py` (4 теста): полный HTTP-цикл email (register→verify→apply, оба получают Pro); неверифицированный email → 400, бонус не начислен; Telegram-нативный пользователь (`email=None`, `email_verified=False`) **не** блокируется гейтом — ключевой тест, доказывающий, что фикс не сломал Telegram; rate-limit на 6-й попытке → 429.
- Полный `pytest` — **154 passed, 6 skipped** (было 150/6, +4 passed, без регрессий).
- `tsc --noEmit` — 0 ошибок.
- Живая ручная проверка в браузере — **выполнена** после того, как Docker поднялся (первая попытка упёрлась в краш Docker Desktop, вторая — в устаревший образ backend без `json-repair`, пересобран `docker compose build backend`). Полный цикл на 2 реальных аккаунтах через настоящий UI: Alice регистрируется → верифицирует email → копирует ссылку `/ref/3ZERN953` → Bob переходит по ней (код лёг в `localStorage`) → регистрируется → верифицирует email → в network-логе браузера виден автоматический `POST /referrals/apply → 200 OK` сразу после `verify-email`. Проверено в БД напрямую: Alice `subscription_tier=pro`, `expires_at=+7д`, `ref_bonus_days_given=7`; Bob `subscription_tier=pro`, `expires_at=+3д`, `referred_by=<Alice.id>`; `referral_log` — 1 строка. В профиле Alice вживую: «Mystral Pro активен · Pro до 18 июля», «Получено дней Pro: +7», счётчик рефералов «1», в списке приглашённых — «Bo\*\*\* +7д».
- Telegram `start_param`-путь — покрыт только `tsc`/код-ревью, не проверен вживую (нужен настоящий Telegram-клиент/бот, недоступно даже с поднятым Docker).

### Next step
- `scheduler.py` `ReferralLog.user_id` краш — уже отдельный тикет-предложение (не в рамках ТЗ-063).
- Тестовые аккаунты `alice@test.com`/`bob@test.com` остались в локальной dev-БД (Docker) — не мешают, можно почистить или оставить для дальнейшего ручного тыканья.
- Остальные находки `AUDIT_2026-07-07.md`: возврат Stars, `/natal` без auth+rate limit, push, языковой пробел, Privacy/Terms — в отдельные тикеты.

---

## 2026-07-11 — Деплой ТЗ-063 упирался в диск на VPS + фикс Dockerfile backend

### Completed
- При деплое ТЗ-063 на прод: `docker compose build` падал на `no space left on device` (сначала на экспорте образа, `journald` тоже не мог писать) — корневой раздел VPS `/dev/vda2` всего 9.8G, был забит на 94%.
- `docker system prune -a -f` + `docker builder prune -a -f` освободили 1.269GB (94%→86% занято) — хватило на фронтенд, но не на backend: тот же файл (`timezonefinder/data/boundaries/coordinates.fbs`) не помещался при экспорте образа снова.
- Причина — `backend/Dockerfile` был однослойный: `build-essential`/`gcc`/`libssl-dev` (нужны только чтобы собрать `pyswisseph`/`cryptography` из исходников) оставались в финальном образе навсегда. Переведён на multi-stage: сборка в отдельном `builder`-стейдже (`pip install --prefix=/install`), в финальный слой копируется только `/install` — без компилятора.

### Verified
- Локально: сборка чистая, content size образа **292MB → 168MB**. Контейнер поднят, планировщик стартовал без ошибок (значит скомпилированные расширения по-прежнему подгружаются корректно — рантайм-либы уже есть в `python:3.11-slim`), `/docs` отвечает 200.
- На проде: `./deploy.sh` прошёл до конца, задеплоилось, коммиты (ТЗ-062, ТЗ-063, этот фикс) на месте — подтверждено пользователем.

### Next step
- Диск на VPS всё ещё около 86% занят даже после чистки — на будущее, если стек продолжит расти, может понадобиться либо расширение диска у провайдера, либо сборка образов вне VPS (registry + pull вместо build на месте). Пока работает, отдельным тикетом не заводил.

## 2026-07-12 — ТЗ-064: возврат Telegram Stars не отзывал Pro-статус

### Completed
- **Диагностика показала две проблемы, не одну.** В `bot/main.py` не было вообще
  никакого обработчика события возврата (в отличие от ТЗ-061, где вебхук
  ЮKassa приходил, но `refund.succeeded` не было в белом списке событий —
  здесь просто нет хендлера). Но глубже: `stars_activate` в
  `backend/app/api/v1/payments.py` никогда не создавал строку в `payments`
  для Stars-покупок — только флоу ЮKassa пишет туда. Значит даже если бы
  обработчик возврата был, искать в БД было бы нечего.
  - Проверил живьём пакет `aiogram==3.10.0` (версия, закреплённая в
    `bot/requirements.txt`) — распаковал wheel, `RefundedPayment` и поле
    `Message.refunded_payment` там уже есть. Апгрейд библиотеки не нужен.
  - По актуальному Bot API возврат Stars-платежа — не отдельный тип апдейта,
    а поле `refunded_payment` на обычном `message`-апдейте (та же механика,
    что у `successful_payment`). Приходит и при обращении пользователя в
    Telegram напрямую, и при вызове ботом `refundStarPayment`. Метод
    `refundStarPayment` принимает только `user_id` + `telegram_payment_charge_id`,
    без суммы — частичных возвратов через него не бывает, но политику
    ТЗ-061 (отзыв Pro при любом возврате, без сравнения сумм) применил и
    здесь для единообразия и на случай, если механизм когда-то изменится.
- **Сделано:**
  - `_record_stars_payment()` (`payments.py`) — пишет `Payment(provider="stars")`
    при активации, идемпотентно по `provider_payment_id=telegram_payment_charge_id`
    (retry активации не даёт дубликат и не падает на уникальном индексе).
    `StarsActivateRequest` получил новое поле `telegram_payment_charge_id`
    (опционально, пусто = старое поведение без записи, с warning в лог).
  - `POST /stars/refund` (внутренний, тот же `_require_internal_token`, что у
    `/stars/activate`) — ищет `Payment` по `telegram_payment_charge_id`,
    отзывает Pro через уже существующий `_revoke_pro` (тот же хелпер, что
    ТЗ-061 завёл для ЮKassa). Идемпотентно: платёж уже `status="refunded"`
    пропускается молча.
  - `bot/main.py`: `on_successful_payment` теперь передаёт
    `telegram_payment_charge_id` в `/stars/activate`; новый хендлер
    `on_refunded_payment` (`F.refunded_payment`) шлёт `charge_id` в
    `/stars/refund` с тем же паттерном retry (3 попытки), что у активации,
    и алертит в поддержку при неудаче — отдельно различая "платёж не найден
    в БД" (старые Stars-покупки до этого фикса, см. ручной шаг ниже) от
    "ошибка сервера".
  - В отличие от вебхука ЮKassa, тут не нужна повторная верификация через
    API провайдера — `refunded_payment` приходит через собственный
    long-polling канал бота, уже аутентифицированный токеном бота, а не
    через публичный незащищённый URL.

### Verified
- Новый класс `TestStarsRefund` (4 теста) в `test_payments.py`: полный цикл
  оплата→Pro→возврат→Pro отозван; идемпотентность повторной доставки;
  неизвестный/несуществующий платёж → 404 без краша; эндпоинт требует
  внутренний токен. Плюс 2 теста на саму запись `Payment` при активации
  (persists row + graceful degradation без charge_id).
  Полный `pytest` — **160 passed, 6 skipped** (было 145, +15 новых, регрессий
  нет). Фронтенд не затронут.

### Manual steps required
- **Настройка Telegram НЕ требуется** (в отличие от ЮKassa, где нужно было
  включить подписку на `refund.succeeded` в личном кабинете) —
  `refunded_payment` приходит как обычный `message`-апдейт, а бот уже
  поллит `message`-апдейты, дополнительный `allowed_updates` не нужен.
- **Все Stars-покупки, сделанные ДО этого фикса, не имеют строки `Payment`**
  (activate их не записывал). Если такая старая покупка будет возвращена,
  бот получит 404 от `/stars/refund`, пришлёт алерт в поддержку с `tg_id`
  из события — Pro для этого пользователя нужно будет отозвать вручную через
  `POST /admin/users/{user_id}/revoke-pro` (тот же путь, что был описан в
  ручных шагах ТЗ-061).

### Next step
- Backfill `Payment` строк для старых Stars-покупок, если найдутся способ
  восстановить `telegram_payment_charge_id` из истории (Telegram Bot API не
  отдаёт список прошлых платежей ботом — скорее всего backfill невозможен,
  остаётся только ручная обработка по факту обращения).
- Alembic-миграции вместо `create_all` — тянется с ТЗ-061, всё ещё в бэклоге.

## 2026-07-12 — ТЗ-065: единая система модерации возвратов (ЮKassa + Stars)

### Completed
- **Контекст:** до этого тикета `/refund-request` (ТЗ-051) только отправлял
  email — не создавал структурированную заявку и не инициировал реальный
  возврат. Реальные возвраты ЮKassa делались вручную через личный кабинет;
  возвратов Stars через код не было вообще. Автоматический отклик на уже
  случившийся возврат (отзыв Pro) — ТЗ-061/ТЗ-064, не трогали, переиспользовали.
- **Новая таблица `RefundRequest`** (`backend/app/models/user.py`):
  `user_id`, `payment_id` (ссылка на `Payment`, работает для обоих
  провайдеров через `provider`), `status` (pending/rejected/completed/failed),
  `reason`, `admin_comment`, `error_detail`, `resolved_at`, `resolved_by`.
  **Решение:** статус `approved` из ТЗ убрал из рантайм-схемы — одобрение и
  вызов провайдера происходят атомарно в одном клике администратора, поэтому
  отдельного "одобрено, но ещё не возвращено" состояния не возникает;
  результат сразу `completed` или `failed`.
  - Модуль 2 (`backend/app/api/v1/payments.py`): `/refund-request` теперь
    сам находит подходящий платёж (`_find_refundable_payment` — последний
    `succeeded` платёж пользователя без открытой/завершённой заявки на него)
    и создаёт `RefundRequest(pending)`. **Решение:** не стал делать
    пользователю выбор платежа из списка (как буквально просили) — на
    практике у пользователя почти всегда один актуальный платёж; авто-выбор
    последнего избавляет от лишнего UI ради редкого кейса. `reason` оставил
    опциональным (как было). Новый `GET /refund-request/status` отдаёт
    статус последней заявки пользователя.
  - Модуль 3 (`backend/app/api/v1/admin.py`): `GET /admin/refunds` (pending +
    история), `POST /admin/refunds/{id}/approve`, `POST /admin/refunds/{id}/reject`
    (обязательный `admin_comment`). Approve вызывает провайдера
    (`yukassa_issue_refund` / `stars_issue_refund` в `payments.py`, реюзаны
    админкой) и переводит заявку в `completed`/`failed` по факту ответа —
    **отзыв Pro НЕ делается здесь напрямую**, а происходит автоматически через
    уже существующие `refund.succeeded`-вебхук (ТЗ-061) / `refunded_payment`
    бот-хендлер (ТЗ-064), чтобы не задваивать идемпотентную логику.
  - Провайдерские вызовы никогда не бросают исключение — возвращают
    `(success, error_detail)`, поэтому ошибка провайдера (например,
    «уже возвращено») превращается в чистый `status=failed` с `error_detail`,
    а не в 500. Дополнительно: если `Payment.status` уже `"refunded"` до
    вызова (возврат сделали в обход системы, вручную в кабинете ЮKassa) —
    короткое замыкание без обращения к провайдеру вообще.
  - Email (`backend/app/core/email.py`): `send_refund_email` расширен
    провайдером/суммой платежа; новый `send_refund_decision_email` —
    «возврат оформлен» либо «отклонён» + `admin_comment`.
- **Фронтенд:**
  - `Profile.tsx` — текстовое поле причины (необязательное) + статус своей
    заявки (pending/rejected+комментарий/completed/failed) над кнопкой,
    подгружается через новый `GET /refund-request/status`.
  - `Admin.tsx` — новая вкладка «Возвраты»: список на рассмотрении (с
    провайдером/суммой/причиной) с кнопками «Одобрить и вернуть»/«Отклонить»
    (комментарий через `window.prompt`, как остальные деструктивные действия
    в этой панели), плюс история решённых заявок.
  - 6 локалей (ru/en/tr/es/pt/uk) — новые ключи `refund_reason_placeholder`,
    `refund_status_*`, `refund_no_payment`.

### Verified
- Новый файл `backend/tests/test_admin_refunds.py` (14 тестов): список
  требует токен; approve ЮKassa (мок) → completed; approve Stars (мок) →
  completed; Stars без привязанного Telegram-аккаунта → failed с понятной
  причиной; ошибка провайдера (400 "already refunded") → failed, не 500;
  уже `refunded` платёж → короткое замыкание, провайдер не вызывается вообще
  (проверено счётчиком вызовов); двойное approve/reject → 400 на второй раз;
  reject без комментария → 422; reject успешно → `rejected` + Pro не тронут.
  `TestRefund` в `test_payments.py` переписан под платёж-ориентированную
  модель (было по `User.subscription_created_at`, стало по `Payment.created_at`)
  + 3 новых теста (нет платежа под Pro → 400, статус пуст изначально,
  статус отражает pending). Полный `pytest` — **177 passed, 6 skipped**
  (было 160, +17 новых, регрессий нет). `tsc --noEmit` — 0 ошибок.
- Фронтенд: поднял dev-сервер (`npm run dev`) и проверил в браузере —
  вкладка «Возвраты» рендерится, пустые состояния корректны
  («Нет заявок на рассмотрении» / «Пусто»), ошибок в консоли нет, Vite-прокси
  верно достаёт до нового `/api/v1/admin/refunds` (просто ECONNREFUSED —
  бэкенда в этой сессии не было, Docker Desktop не запущен). Живой e2e-прогон
  через реальный бэкенд (оплата → заявка → approve → реальный вызов
  ЮKassa/Telegram → вебхук отзывает Pro) **не выполнялся** — недоступен
  Postgres/Redis в этой сессии. Логика полностью покрыта автотестами, но
  живая проверка на VPS/локальном docker-compose — must-do перед тем, как
  считать фичу боевой.

### Manual steps required
- Живой прогон полного цикла на реальном стенде (см. выше) — Docker не
  поднимался в этой сессии.
- Ничего нового на стороне ЮKassa/Telegram настраивать не нужно — approve
  использует те же ключи/токен, что уже были в ТЗ-038b/055/064.

### Next step
- Живой e2e-прогон на VPS/docker-compose перед тем, как полагаться на фичу
  в проде.
- Alembic-миграции вместо `create_all` — всё ещё в бэклоге (ТЗ-061/064/065
  все трогают схему через `create_all`).

## 2026-07-12 — ТЗ-065 доп.: живой прогон на docker-compose нашёл 2 реальных бага

- Саша поднял Docker Desktop — прогнал полный цикл через реальные
  Postgres/Redis/backend (не SQLite из тестов). Живые находки:
  1. **`DELETE /admin/users/{id}` падал 500** на любом пользователе с
     платежом — ручной cascade-список в `delete_user` (`backend/app/api/v1/admin.py`)
     никогда не включал `payments` (пробел с ТЗ-038b/055, просто не был
     покрыт тестами) и тем более не знал про новый `refund_requests`.
     Postgres реально проверяет FK (`payments_user_id_fkey`), SQLite в
     тестах — нет, поэтому пробел был невидим до живого прогона.
  2. **Тот же cascade-цикл никогда не удалял ни одной строки на SQLite** —
     сырой `text("DELETE ... WHERE user_id = :uid")` с `{"uid": user_id}`
     (питоновский `UUID`) падает на aiosqlite ("type 'UUID' is not
     supported"); `str(user_id)` (с дефисами) не падает, но и не матчится —
     SQLite хранит UUID-колонки как 32-символьный hex БЕЗ дефисов.
     Эмпирически проверил формат хранения и поправил на `user_id.hex` —
     подходит и SQLite (совпадает формат), и Postgres (uuid-тип принимает
     hex без дефисов на входе).
  - Добавлен `TestDeleteUserCascade` (`test_admin_refunds.py`) — раньше
    `delete_user` вообще не имел тестового покрытия. Полный `pytest` —
    **178 passed, 6 skipped**.
  - Отдельно, во время живой проверки заметил: у этого бэкенда кастомный
    global exception handler (`main.py`) отдаёт ошибки как
    `{"error": status, "message": "..."}`, а не FastAPI-дефолтный
    `{"detail": "..."}`. Моя новая строка в `Profile.tsx` читала `d.detail`
    (никогда не сработало бы) — поправил на `d.message || d.detail`. Это
    системный паттерн по всему фронтенду (LoginScreen, NatalChart,
    ForgotPassword, ResetPassword и другие места в самом Profile.tsx читают
    `.detail` тем же образом) — чинил только свою новую строку, остальное
    вне периметра этого тикета.
  - Живьём подтверждён полный цикл модерации: заявка → статус в профиле →
    список в админке → reject с комментарием (+ защита от повторного) →
    короткое замыкание на уже возвращённом платеже (+ защита от повторного
    approve) → каскадное удаление пользователя с платежами и заявками.
  - **Не проверялся** живой успешный `approve` с реальным списанием у
    провайдера — auto-mode классификатор корректно заблокировал попытку
    дёрнуть боевые API ЮKassa/Telegram фиктивными ID платежей ("Real World
    Transactions"), поскольку это реальный вызов финансового провайдера
    боевыми ключами, на который Саша явно не давал добро (просил "прогнать
    локально", не "вызвать провайдера"). Эта ветка остаётся проверенной
    только моками в автотестах.
  - Тестовые пользователи/платежи, созданные для проверки, удалены из БД.
    `.env` не имел `ADMIN_TOKEN` вообще (админка и внутренние эндпоинты бота
    были недоступны локально даже без учёта ТЗ-065) — по просьбе Саши
    оставил токен настроенным (сгенерирован `secrets.token_urlsafe(32)`).
    Docker-контейнеры остановлены после проверки по его просьбе.

## 2026-07-13 — Ежедневный гороскоп: обрыв текста рецидивировал + Markdown-«**» пролезал в HTML

- **По двум скринам от Саши** (push-уведомления "Гороскоп на сегодня"):
  1. Текст обрывался на полуслове у Скорпиона ("...доведи" и всё) —
     подозрительно похоже на баг, уже чинившийся 8 июля (`GENERATION_MAX_TOKENS`
     200→500). Попросил Сашу проверить прод-логи — подтвердилось:
     `Daily horoscope for scorpio/ru truncated by max_tokens
     (finish_reason=length)`, 2026-07-13 06:00 UTC. 500 всё равно не хватило.
  2. У Овна литерально показывалось `**работа**` вместо жирного текста.
- **Root cause обрыва (не гадал, поднял историю ТЗ-060):** тот же класс бага,
  что там чинили трижды подряд — модель тратит бюджет токенов на
  саморассуждение/самокоррекцию прямо в тексте ответа до того, как выдаст
  финальный вариант, а не только кириллица требует больше токенов на слово.
  Урок ТЗ-060 (`ece3796`, `ff82a3a`, `09e8524`) — перестать подбирать число по
  чуть-чуть и (а) явно запретить рассуждения вслух в промпте, (б) дать щедрый
  потолок токенов, раз это не стоит дороже при более коротком реальном ответе.
- **Root cause Markdown:** рассылка шлётся с `parse_mode: "HTML"`
  (`app/scheduler.py`), а системный промпт нигде не запрещал модели
  Markdown-разметку — HTML-режим Telegram `**bold**` не понимает вообще,
  показывает как есть.
- **Сделано:**
  - `backend/app/services/horoscope.py`: `GENERATION_MAX_TOKENS` 500 → 2000
    (щедрый потолок, не дороже при коротком ответе). В промпт (ru и en/прочие
    ветки) добавлен явный запрет на рассуждения вслух/самокоррекцию
    ("хотя нет", "точнее", "wait, actually") и на Markdown-разметку —
    формулировка взята из уже проверенного анти-rambling промпта SEO-генерации.
  - `backend/app/scheduler.py`: новая `_sanitize_llm_text()` — подстраховка
    поверх промпт-фикса на случай, если модель всё равно проигнорирует
    инструкцию: экранирует HTML-метасимволы (иначе случайный `<`/`&` от модели
    мог бы вообще сломать парсинг Telegram, не просто криво отрендериться) и
    конвертирует `**bold**` → `<b>bold</b>`. Применяется в `_format_message`
    до сборки финального сообщения.
- **Проверено:** `test_horoscope_service.py` — порог поднят до `>= 2000`,
  новый тест на наличие анти-Markdown инструкции в промпте (оба языковых
  пути). Новый `test_scheduler.py` (4 теста) — конвертация `**bold**`,
  обычный текст не трогается, HTML-метасимволы экранируются, несколько
  bold-фрагментов в одном тексте. Полный `pytest` — **183 passed, 6 skipped**
  (было 178, регрессий нет).
- **Требуется деплой** — фикс только в git, на VPS ещё не выкатан
  (`./deploy.sh`), реальный эффект в проде появится после следующего деплоя.

## 2026-07-13 — ТЗ-059: регистрация — generic "Error", валидация имени, баг ввода времени

### Модуль 1 — generic "Error" вместо реальной причины
- **Root cause:** `LoginScreen.tsx` читал `data.detail` из ответа API, но
  глобальный exception handler бэкенда (`main.py`) отдаёт ошибки как
  `{"error": status, "message": "..."}`, а не FastAPI-дефолтный
  `{"detail": "..."}` — `data.detail` всегда `undefined`, поэтому всегда
  срабатывал fallback `"Error"`. Тот же системный паттерн уже находил и чинил
  точечно в `Profile.tsx` (ТЗ-065-доп.) — здесь это было explicit-в-периметре
  тикета, поэтому почищено полностью в `LoginScreen.tsx` (оба пути: обычная
  регистрация/логин и Telegram-виджет).
  - Разовый обрыв 400 без деталей (о котором писал Саша) — не воспроизвёл и
    не гадал вслепую (как и просили): `_check_ip_rate` отдаёт 429, не 400, так
    что это не rate-limit; сам факт, что реальный текст теперь виден, значит
    при повторном возникновении причина будет сразу читаема, а не потеряна.
  - `res.json()` обёрнут в `.catch(() => ({}))` — если тело не JSON вообще
    (например, апстрим отдал HTML 502-страницу), fallback теперь на общий
    текст (`login.generic_error`, новый i18n-ключ, 6 языков), а не сырой
    `SyntaxError` от `JSON.parse`.
### Модуль 2 — валидация имени
- Было: только `!name.trim()` и `length > 100`. Новое (frontend `validate.ts`
  + backend `_validate_name` в `auth.py`, одинаковые правила в двух местах):
  не пусто после trim, 2-50 символов, минимум одна буква (frontend:
  `/\p{L}/u`; backend: `str.isalpha()` — стандартный эквивалент без
  сторонней `regex`-библиотеки, работает для любого из 6 языков платформы,
  не хардкод конкретных алфавитов), запрет `<`/`>` (XSS в шер-карточках/
  письмах/профиле). **Блеклист "SQL-символов" сознательно не делал** —
  проверил, что `display_name`/имя нигде не конкатенируется в сырой SQL
  (только через ORM, параметризовано) — блеклист просто отсекал бы легитимные
  имена без реальной пользы.
  - Фронтенд: чек-лист под полем имени в реальном времени, по образцу уже
    существующего чек-листа пароля на этой же форме (2 критерия: длина,
    наличие буквы), плюс кнопка submit заблокирована при невалидном имени —
    аналогично паролю.
  - Имя триммится перед сохранением в БД (`req.name.strip()`).
### Модуль 3 — не вводилось время рождения (диагностика → фикс)
- **Диагностика:** поле в `OnboardingModal.tsx` (шаг 1 регистрационного
  флоу онбординга, не сама форма регистрации — время рождения там не
  запрашивается вообще). Реальный `<input type="time">`, но **скрытый**
  (`opacity: 0, pointerEvents: "none"`) и открывался программно через
  `timeRef.current?.showPicker?.()` по клику на отдельную кнопку.
  **Root cause:** `showPicker()` поддерживается не везде (Safari < 16.4,
  вероятно встроенный WebView Telegram на более старых iOS) — при отсутствии
  метода `?.()` молча ничего не делает, без ошибок в консоли. День/месяц/год
  рядом используют другой, надёжный паттерн — реальный `<select>` поверх
  кастомного вида, кликабельный напрямую, без зависимости от JS API. Поле
  времени было единственным местом с `showPicker()`-паттерном во всём фронтенде.
- **Фикс:** убрал скрытый инпут + `showPicker()` целиком, поле времени
  теперь использует ТОТ ЖЕ overlay-паттерн, что день/месяц/год — реальный
  `<input type="time">` поверх стилизованного блока, клик работает через
  штатное поведение браузера с любым native time-контролом, не через
  скриптовый API. Заодно сделал инпут `controlled` (`value=...`) и добавил
  очистку `hour`/`minute` при очистке поля (была невозможна раньше).
### Тесты и проверка
- Backend: 4 новых теста в `test_auth.py` (пустое/только пробелы/1 символ,
  спецсимволы-без-буквы/51+ символов/HTML-тег — оба набора разбиты на 2 теста
  из-за лимита 5 регистраций/час/IP; апостроф/дефис/кириллица/составные имена
  разрешены; имя триммится при сохранении). Один существующий тест
  (`test_profile.py::TestReferralFullCycle`) использовал имя `"N"` (1 символ) —
  поправлен на `"Nina"`, поскольку это теперь легитимно невалидно, а не баг.
  Полный `pytest` — **194 passed, 6 skipped** (было 190). `tsc --noEmit` — 0 ошибок.
- Живая проверка на docker-compose (реальный Postgres/Redis/backend):
  повторная регистрация с тем же email → в UI показалось настоящее
  "Email already registered" вместо "Error"; чек-лист имени реагирует на
  каждый символ вживую; `<script>alert(1)</script>` отклонён бэкендом с
  понятным текстом, `O'Brien` — принят; поле времени в онбординге теперь
  распознаётся как настоящий интерактивный `textbox type="time"` (было
  полностью инертным), значение `14:30` корректно вводится и отражается в UI.
  Тестовые аккаунты удалены после проверки, Docker остановлен.

---

## 2026-07-14 — ТЗ-070/071: кнопка "Рассчитать карту" невидимо disabled вне Telegram

### Completed
- **Диагностика (ТЗ-070, код-ревью без устройства):** `Telegram.WebApp`-gating,
  геолокация, viewport-медиазапросы для `pointer-events` — не найдены,
  исключены как причина. Найдена реальная причина: `canSubmit` в
  `NatalChart.tsx` требует `form.name`, а `birth_name` не сохраняется в
  онбординге (`OnboardingModal.saveAllData()`) — только через отдельное
  поле в Профиле или после первого успешного расчёта. Кнопка при этом
  реально `disabled`, но `Button.tsx` не имел никакого стиля для
  disabled-состояния — золотой градиент рисовался одинаково что для
  активной, что для заблокированной кнопки, поэтому "нет реакции на тап"
  выглядело как баг обработчика, а не как корректно заблокированная форма.
  Подтверждено вручную: после ручного ввода имени форма заработала.
- **Фикс (ТЗ-071), модуль 1:** `Button.tsx` — при `disabled` добавлены
  `opacity: 0.45`, `filter: grayscale(.5)`, `cursor: not-allowed`,
  `boxShadow: none` (снимает золотое свечение); shimmer-анимация отключена
  на disabled. Стиль общий для всех вариантов кнопки, применяется
  автоматически везде, где используется `<Button>` (Profile, Compatibility,
  Runes, Tarot, LunarCalendar, LoginScreen, MergeAccountPrompt и т.д.) —
  отдельно не трогал.
- **Фикс, модуль 2:** `NatalChart.tsx` — под кнопкой "Рассчитать карту",
  пока форма не заполнена, показывается подсказка "Заполните: Имя, Город..."
  с кликабельными названиями полей — клик скроллит и фокусирует нужный
  инпут (`ref` на каждое из 5 обязательных полей). Хардкод ru/en-текста
  подсказки — по образцу уже принятого в этом файле паттерна
  (`elLabels`/`sectionLabels`), названия полей берутся из существующих
  i18n-ключей (`t("natal.name")` и т.д.), новые ключи не добавлялись.
- **Модуль 3 (обсуждение, не реализовано):** перенос сбора `birth_name` в
  онбординг/саму форму — не сделан, поле и так уже редактируется прямо на
  странице натальной карты (не только в Профиле, как предполагалось в ТЗ) —
  открытый вопрос для отдельного продуктового решения, не блокирует.

### Problems found
- При ревью других мест с `disabled` в проекте — как минимум
  `Profile.tsx` (кнопка удаления аккаунта, строка ~795) и `Numerology.tsx`
  (кнопка AI, строка ~355) используют "голый" `<button>` без стиля
  disabled вообще (тот же класс бага) — не в скоупе этого ТЗ (оно
  ограничено `Button.tsx`), не тронуто.

### Verified
- `tsc --noEmit` — 0 ошибок.
- Живая проверка через dev-сервер (`npm run dev`, без Docker/бэкенда —
  недоступен в этой сессии): на `LoginScreen` (тоже использует `<Button>`)
  переключение в режим регистрации с пустой формой → submit-кнопка
  реально `disabled`, `style` атрибут подтверждён напрямую через DOM —
  `opacity: 0.45; filter: grayscale(0.5); cursor: not-allowed;` и без
  золотого box-shadow; после заполнения валидных полей — `disabled: false`,
  обычный вид с свечением. Саму страницу натальной карты (нужен логин)
  живьём не проверял — бэкенд/Docker не запущены в этой среде.

### Next step
- Проверить вживую на реальном сценарии из критериев готовности: новый
  тестовый аккаунт → онбординг → сразу натальная карта → подсказка
  показывает "Имя", клик по ней фокусирует поле, после ввода — успешный
  расчёт.
- Решить (отдельно, не блокирует): стоит ли закрыть тем же паттерном
  disabled-стиля `Profile.tsx`/`Numerology.tsx` из "Problems found" —
  тот же класс бага, но вне скоупа ТЗ-071.

---

## 2026-07-14 — ТЗ-072: Privacy.tsx и Terms.tsx противоречили друг другу по сроку хранения данных

### Completed
- Найденное в аудите (ещё в записи от 2026-07-08) противоречие подтверждено
  и исправлено: `Privacy.tsx` (раздел 4 «Хранение данных») утверждал
  **12 месяцев** после удаления аккаунта, `Terms.tsx` (раздел 8
  «Расторжение») — **30 дней**.
- Сверено с кодом (тот же путь, что живьём проверяли в ТЗ-065):
  `auth.py::delete_account` ставит `deletion_scheduled_at = utcnow() + 30
  дней`; `scheduler.py::cleanup_deleted_accounts` — каскадное удаление
  пользователя и всех связанных записей (TarotReading, RuneReading, Review,
  UserPartner, ReferralLog, UserProfile, AuthProvider), задача
  зарегистрирована в `main.py` и реально крутится ежедневно в 03:00 UTC.
  **30 дней — подтверждённое текущее поведение**, признаков того, что
  12 месяцев было намеренным решением (в коде/комментариях/PROGRESS.md), не
  найдено — код принят за источник истины.
- `Privacy.tsx`: «12 месяцев»/«12 months» → «30 дней»/«30 days» (ru+en),
  точечная правка одного числа в разделе 4, без переписывания остального
  текста.

### Problems found
- Других противоречий между `Privacy.tsx` и `Terms.tsx` не найдено — сверил
  оба документа целиком раздел за разделом (оператор/ИНН/email, сроки
  обработки запросов, права пользователя, данные, которые собираются). Раздел
  6 Privacy.tsx («запрос обработан в течение 30 дней») теперь согласуется и с
  Terms.tsx, и с самим собой — до фикса «12 месяцев» в разделе 4 не совпадало
  даже внутри одного документа.

### Verified
- `tsc --noEmit` — 0 ошибок.
- Живая проверка через dev-сервер: `/privacy` — публичный роут, доступен без
  логина; текст на странице подтверждён через `get_page_text` — раздел 4
  показывает «...или через 30 дней после удаления аккаунта».

### Next step
- Не коммитил/не пушил — ждать явного запроса, как и раньше в этой сессии.

---

## 2026-07-15 — ТЗ-073: 21 SEO-страница (12 рун + вся нумерология) отдавала 500

### Completed
- Воспроизвёл на реальном ASGI-клиенте (не догадка): у `_generate_and_store`
  (`backend/app/core/seo_generator.py`) шаг записи в кеш (`session.exec` +
  `session.add` + `session.commit`) стоял **вне** try/except, в отличие от
  шага вызова LLM прямо над ним. Смонтировал фейковый LLM-клиент,
  возвращающий валидный JSON, и заставил `AsyncSession.commit()` бросить
  исключение — страница `/runes/eihwaz` реально упала 500 (полный traceback
  зафиксирован), хотя контент уже был успешно сгенерирован. Это прямое
  нарушение собственного контракта функции («returns parsed content... or a
  fallback — fallbacks are never persisted», never raise) — единственное
  необёрнутое место во всей функции.
- Числа сошлись до знака: `seo_page_items()` отдаёт плоский список zodiac(12)
  + tarot(78) + rune(24) + numerology(9) = 123 — ровно позиции 103–123 этого
  списка (последние 12 рун + вся нумерология = 21 штука) и есть найденные
  краулером «сломанные» страницы. Это указывает на `warm_seo_cache()`
  (см. Problems found) как на то, почему именно эти 21 страница вообще
  оказались не прогреты и зависели от рискованного on-demand пути — но сам
  503/500 чинит именно фикс try/except выше, а не это.
- Фикс: обернул шаг персиста в `_generate_and_store` в свой try/except —
  при сбое записи в кеш страница всё равно рендерится с уже сгенерированным
  реальным контентом (просто не закешируется, перегенерируется в следующий
  раз), вместо необработанного 500.
- Данные-источники (`seo_data.py`: `RUNE_SEO` — все 24, `NUMEROLOGY_SEO` —
  все 9; `seo_i18n.py`: `_RUNE_LATIN`/`_RUNE_UK`/`_NUM_NAMES` — по 24/9 ключей
  на каждый язык) проверены построчно — никакой обрезки данных нет, версия
  «TZ-037c батч-скрипт обрезал структуру» не подтвердилась.

### Problems found
- `warm_seo_cache()` (тот же файл) после 3 подряд ошибок делает `return` —
  бросает прогрев **всех** оставшихся страниц во всех типах разом, а не
  только текущего типа. Комментарий из коммита `52ff482` («No point burning
  through error responses when Groq daily limit is hit... resumes next
  server restart») явно предполагал, что непрогретые страницы спокойно
  переживут on-demand генерацию до следующего рестарта — но это предположение
  было неверным именно из-за бага выше, отчего 21 страница вслед за первым
  сбоем оказались 500, а не просто «медленнее генерируются». Не трогал —
  сам по себе этот `return` не 500-ит (это вопрос охвата прогрева, не
  корректности), а фикс try/except выше уже полностью закрывает
  зарепорченный симптом. Флагирую отдельно, если Саша захочет сделать прогрев
  устойчивее к точечным сбоям.

### Verified
- Полный `pytest`: **393 passed, 6 skipped** (без Docker — окружение то же,
  что и раньше в этой сессии; SQLite вместо Postgres, как в остальных
  бэкенд-тестах).
- Новый регрессионный тест (`backend/tests/test_seo_pages.py`,
  `TestAllRunesAndNumerologyPages`) проходит по **всем** 24 slug рун и всем
  9 slug нумерологии — на `ru` и на всех 5 префиксных языках (en/es/pt/tr/uk),
  не только на одном примере, как раньше (`test_rune_returns_html` покрывал
  только `fehu`). Отдельный тест
  (`TestSeoContentPersistenceFailureDoesNotCrashPage`) закрепляет сам фикс:
  сбой `session.commit()` больше не роняет страницу.
- Фронтенд не затронут — `tsc --noEmit` не запускал (незачем).

### Next step
- Живая curl-проверка на реальном проде (`mystral.space/runes/eihwaz` и
  т.д.) не выполнялась — нет доступа к продовой среде из этой сессии;
  проверено эквивалентно через ASGI test-client, который проходит через тот
  же самый роутинг/middleware.
- Не коммитил/не пушил — ждать явного запроса.

---

## 2026-07-15 — ТЗ-075: рассинхрон истечение Pro / уведомление / отображение в админке

(продолжение диагностики того же дня — см. предыдущую диагностическую сессию
без записи в PROGRESS.md, отвечал в чате: единственный механизм понижения
tier был ленивый, в `deps.get_current_user`, только при заходе самого
пользователя; уведомление «истекла» слалось по `.date()`, не по факту
отключения; админка отдавала сырой `tier` из БД без пересчёта)

### Completed
- **Активный job понижения** — `demote_expired_subscriptions()`
  (`backend/app/scheduler.py`), зарегистрирован в `main.py` — cron, каждый
  час (`minute=0`). Находит всех `tier="pro"` с `subscription_expires_at <
  utcnow()`, понижает до `free` (`expires_at = None`), коммитит — независимо
  от того, заходил ли пользователь.
- **Уведомление «истекла» синхронизировано с реальным моментом** — перенёс
  его из `send_subscription_reminders` (слалось по `exp_date == today`,
  дата без времени) в сам `demote_expired_subscriptions` — теперь шлётся
  сразу после реального понижения конкретного пользователя, а не по
  отдельной проверке даты. Дедуп через Redis-ключ не понадобился: после
  понижения пользователь больше не попадает в выборку следующего прогона —
  естественная идемпотентность. `send_subscription_reminders` оставил только
  предупреждение «за 3 дня» (оно осознанно date-based — это заранее, а не
  факт) и обновил докстринг, явно объясняющий это разделение.
- **Админка пересчитывает статус на лету** — `GET /admin/users`
  (`backend/app/api/v1/admin.py`) больше не отдаёт сырой `u.subscription_tier`:
  если `tier == "pro"` и `expires_at < utcnow()`, в ответе API подставляется
  `"free"` — без записи в БД (запись поправит новый job или ленивая проверка
  при следующем входе). Заодно тем же способом поправил `GET /admin/stats`
  (`pro_users` count) — той же болезнью страдал: считал по сырому `tier`, не
  проверяя `expires_at`; в ТЗ не был явно назван, но это тот же класс бага
  на той же странице админки, и точечно дешёвый фикс — не оставил в
  рассинхроне рядом с уже почищенным местом.
- **UI показывает время, не только дату** — `Admin.tsx` (обе карточки —
  мобильная и десктоп-таблица, общий хелпер `formatExpiresAt`) и
  `Profile.tsx` (`profile.pro_until`) — вместо `toLocaleDateString` теперь
  `toLocaleString`/`toLocaleDateString` с `hour`/`minute`, например
  «до 15.07.2026, 15:12» вместо «до 15.07.2026».

### Problems found
- Ничего нового вне периметра — единственное расширение периметра было
  осознанным (см. `admin_stats` выше, тот же класс бага что и `admin_users`).

### Verified
- `tsc --noEmit` — 0 ошибок.
- Полный `pytest` — **402 passed, 6 skipped** (было 393 до ТЗ-075, +9 новых:
  4 на `demote_expired_subscriptions`, 2 на обновлённый
  `send_subscription_reminders`, 3 на пересчёт статуса в `admin_users`/`admin_stats`).
  Новые тесты покрывают: истёкший pro-пользователь реально понижается +
  получает уведомление; активный — не трогается; понижение работает и без
  telegram-токена (не должно зависеть от доставки уведомления); пустая
  таблица — no-op; 3-дневное предупреждение всё ещё шлётся; уведомление
  «истекла» по дате больше НЕ шлётся из старого job'а (перенесено); админский
  список и статистика показывают пересчитанный статус, а сырая запись в БД
  при этом не меняется (проверено отдельным чтением через `AsyncSessionLocal`).

### Next step
- Живая проверка на реальном проде после деплоя (совпадает с проверкой,
  которую Саша делает вручную для ТЗ-073) не выполнялась — нет доступа к
  продовой среде из этой сессии.
- Не коммитил/не пушил — ждать явного запроса.

## 2026-07-17 — ТЗ-082: регресс 500 на RU numerology/runes (12 рун + все 9 нумерологии)

### Completed
- **Найдена точная причина** — та же болезнь, что чинили в ТЗ-073, но на
  соседнем месте, которое ТЗ-073 не задело. `get_seo_content()`
  (`backend/app/core/seo_generator.py`) делает начальный SELECT в
  `SeoContent` ДО того, как понятно — кэш есть или нет; этот SELECT висел
  вне try/except, в отличие от пары SELECT+commit внутри
  `_generate_and_store`, которую ТЗ-073 уже обернул. Временный сбой БД
  (исчерпание пула соединений, разрыв соединения) на этом первом чтении
  падал наружу как необработанный 500 — именно на тех 21 страницах, которые
  ТЗ-073 identified как единственные, чей on-demand путь держит соединение
  открытым на время живого обращения к LLM (12 последних рун + все 9
  нумерологии — ровно те, что `warm_seo_cache()` в своё время не прогрел
  из-за abort-after-3-errors, см. заметку в коммите ТЗ-073).
- **Фикс** — обернул тот же начальный SELECT в try/except: при сбое чтения
  кэша просто логируем и падаем в generate-с-нуля вместо краша страницы.
  Минимальный точечный патч, зеркалит уже принятый паттерн из ТЗ-073.
- **Второй фикс, найденный при ответе на контрольный вопрос (см. ниже)** —
  в except-блок добавлен `await session.rollback()`. На реальном Postgres
  (не SQLite) сбойный SELECT оставляет транзакцию сессии в состоянии
  aborted; без явного rollback ЛЮБОЙ следующий вызов на той же сессии —
  включая SELECT+INSERT персистентности внутри `_generate_and_store`,
  который выполняется чуть ниже на той же самой session — падает с
  `asyncpg.exceptions.InFailedSQLTransactionError`. Без этого второго фикса
  страница всё ещё не падала бы (её ловит уже существующий guard ТЗ-073
  вокруг persist-шага), но свежесгенерированный контент в рамках ЭТОГО
  запроса в кэш бы не попадал.
- **Regression-тест** — `TestSeoContentCacheLookupFailureDoesNotCrashPage`
  в `test_seo_pages.py`, мокает `AsyncSession.exec` с исключением на первом
  чтении (тот же приём, что и в тесте ТЗ-073 для commit): без фикса тест
  падает тем же необработанным 500, что видели на проде; с фиксом — 200 с
  реальным сгенерированным контентом. Существующий `TestAllRunesAndNumerologyPages`
  (ТЗ-073, полный обход всех 24 рун + 9 чисел на ru и всех 5 префиксов) не
  трогал — он уже покрывает регресс на уровне «страница отвечает 200».

### Problems found
- Живая проверка прод-URL (21 адрес, до 105 запросов включая параллельные)
  на момент сессии 500 не воспроизвела — все страницы уже закэшированы
  реальным контентом (не fallback), отвечают быстро. Симптом, видимо,
  самоизлечился естественным трафиком после первого TZ-073-фикса (страница
  один раз успешно сгенерировалась и закэшировалась). Само отсутствие
  защиты на первом SELECT — реальный, подтверждённый тестом баг независимо
  от текущего состояния прод-кэша.

### Ответы на контрольные вопросы перед закрытием
1. **Дописывается ли fallback-результат обратно в кэш?** Смотря какой сбой.
   Изначально (без `session.rollback()`) — эмпирически проверено на живом
   Postgres в Docker (`_probe_session_state.py`, одноразовый скрипт, не
   коммитился): после сбойного стейтмента ЛЮБОЙ следующий вызов на той же
   сессии падает с `InFailedSQLTransactionError`, то есть persist-шаг внутри
   `_generate_and_store` тоже падает (его ловит существующий guard ТЗ-073,
   страница всё равно отдаёт 200), но контент НЕ кэшируется — при
   сохраняющемся сбое БД каждый повторный запрос будет заново дёргать LLM.
   После добавления `session.rollback()` в except-блок (см. выше) —
   проверено тем же способом (`_probe_selfheal.py`, тоже одноразовый,
   удалён): сессия восстанавливается, persist-шаг в рамках ТОГО ЖЕ запроса
   проходит успешно, строка реально появляется в `seo_content`. Т.е. с
   финальным фиксом — да, дописывается, в том же запросе, если только
   исходный сбой был временным (сама LLM-генерация всё равно не кэшируется,
   если это честный `_fallback()`, — это осознанное поведение, не баг).
2. **Проверила ли live-Docker-проверка сам механизм интермиттентности
   («LLM-вызов держит открытым соединение с БД»)?** Нет. Локальный
   `OPENROUTER_API_KEY` пустой, поэтому в Docker-проверке on-demand путь
   падал в `_fallback()` почти мгновенно (401 от OpenRouter до генерации
   токенов) — ни разу не было живого многосекундного LLM-вызова, держащего
   сессию открытой. Сам механизм удержания соединения во время внешнего
   HTTP-вызова подтверждён только чтением кода (session передаётся в
   `_generate_and_store` и используется до и после `await
   client.chat.completions.create(...)`), не воспроизведён под нагрузкой.
   Docker-часть проверки подтверждает: (а) весь путь работает на реальном
   Postgres без 500, (б) fallback-ветка и её non-persistence ведут себя как
   ожидается. Юнит-тесты и оба точечных Postgres-probe'а (см. вопрос 1)
   подтверждают само поведение сессии/транзакции при сбое независимо от
   того, чем сбой вызван — но не то, что затянувшийся LLM-вызов реально
   исчерпывает пул под нагрузкой на проде. Это остаётся правдоподобной,
   согласующейся со всеми уликами (точное совпадение 21-страничного
   отпечатка с ТЗ-073), но не выполненной под нагрузкой гипотезой.

### Verified
- Полный `pytest` — **544 passed, 6 skipped**, без регрессий (после обоих
  фиксов, включая `session.rollback()`).
- Живая проверка в Docker на реальном Postgres (`docker-compose up -d
  postgres redis backend nginx`, локальная БД пустая — 0 строк в
  `seo_content`): все 9 numerology + все 24 rune RU-урла напрямую на
  `localhost:8000` — 200 у всех, без 500, без исключений в логах backend
  (OPENROUTER_API_KEY в локальном `.env` пустой, так что on-demand
  generation безопасно падает в fallback без реального платного вызова —
  ни разу не спровоцировал настоящую AI-генерацию).
- Отдельно, тем же Docker+Postgres, двумя одноразовыми скриптами внутри
  `mystral-backend-1` (не коммитились, удалены после прогона): эмпирически
  подтверждено поведение сессии из вопроса 1 выше (aborted-транзакция без
  rollback → `InFailedSQLTransactionError` на следующем вызове; с rollback
  → восстанавливается и реально пишет строку в БД).
- Контейнеры остановлены после проверки (`docker-compose stop`).

### Next step
- Не коммитил/не пушил — ждать явного запроса.
- Деплой на прод и последующий шаг (переиндексация GSC + пересдача sitemap
  в Яндекс.Вебмастер для 21 URL) — ручные действия Саши, вне доступа этой
  сессии; краткий runbook даю в чате.

