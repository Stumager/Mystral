        ...
        {items.map(({ id, label, icon }) => {
          ...
          return (
            <button key={id} onClick={() => handleClick(id)}
              ...
              <span style={{ fontSize: 22, lineHeight: 1, color: isActive ? "#E8CD7E" : "#827A69", filter: isActive ? "drop-shadow(0 0 10px rgba(201,168,76,.7))" : "none" }}>{icon}</span>
              <span className="font-cinzel" style={{ fontSize: 8, letterSpacing: ".1em", color: isActive ? "#E8CD7E" : "#827A69", fontWeight: isActive ? 600 : 400 }}>{label}</span>
            </button>
          );
        })}
      </div>
