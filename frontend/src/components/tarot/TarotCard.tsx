...
export function TarotCard({ cardId, name, revealed, reversed, delay = 0 }: TarotCardProps) {
  ...
  if (flat) {
    return (
      <div
        style={{
          width: 88, height: 144, borderRadius: 10, overflow: "hidden",
          position: "relative",
          border: reversed ? "2px solid rgba(239,68,68,0.5)" : "1px solid rgba(140,110,255,0.35)",
          boxShadow: "0 8px 24px rgba(107,78,255,0.3), 0 2px 8px rgba(0,0,0,0.5)",
        }}
      >
        <img
        ...
        {reversed && (
          <div style={{ position: "absolute", top: 4, left: 0, right: 0, display: "flex", justifyContent: "center" }}>
            <span style={{ fontSize: 7, color: "#fff", background: "rgba(239,68,68,0.8)", padding: "1px 5px", borderRadius: 4 }}>REV</span>
          </div>
        )}
      </div>
    );
